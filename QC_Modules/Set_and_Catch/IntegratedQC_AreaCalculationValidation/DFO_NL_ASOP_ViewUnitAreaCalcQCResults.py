...
def ViewCalcValidatedResult_UnitArea():
    windows = tk.Toplevel()
    windows.title ("DFO-NL-ASOP Unit Area Validator - ID-D-03-2")
    ...
    lbl_SortLabel = Label(HeaderFrame, font=('aerial', 11, 'bold'),
            bg= "cadet blue", text="UnitArea QC Table: ")
    ...
    def get_Unit_AreaQC_dfDB():
        ...
        try:
            ...
            cursor = sqliteConnection.cursor()
            Unit_AreaQC_df = pd.read_sql_query("SELECT * FROM SetCatch_UnitAreaQCAnalysis;", sqliteConnection)
            length_Unit_AreaQC_df = len(Unit_AreaQC_df)
            if length_Unit_AreaQC_df > 0:
                Unit_AreaQC_df = Unit_AreaQC_df[
                ((Unit_AreaQC_df.UAValidityCheck_StartPoints) == 'UnitArea-QC Failed')]
                
                Unit_AreaQC_df['Assigned_UnitAreaPolygon'] = (Unit_AreaQC_df
                ['Assigned_UnitAreaPolygon'].map(lambda x: str(x)[0:7])) + ' - ' +\
                 Unit_AreaQC_df['UnitArea']
                
                Unit_AreaQC_df = Unit_AreaQC_df.loc[:,
                ['DeploymentUID', 'UAValidityCheck_StartPoints', 'UAValidityCheck_EndPoints',
                 'StartPoints','EndPoints', 'Assigned_UnitAreaPolygon', 
                 'Calculated_UnitAreaPolygon','RecordType']]
                Unit_AreaQC_df = Unit_AreaQC_df.reset_index(drop=True)
                Unit_AreaQC_dfDB = pd.DataFrame(Unit_AreaQC_df)
                Unit_AreaQC_dfDB=  Unit_AreaQC_dfDB.merge(
                    Return_ObsSetCatchDB, 
                    on = ['DeploymentUID', 'RecordType'],
                    indicator=True, 
                    how='left').query('_merge == "both"')

                lengthFlagged = len(Unit_AreaQC_dfDB)
                sqliteConnection.commit()
                return Unit_AreaQC_dfDB, length_Unit_AreaQC_df, lengthFlagged
            else:
                messagebox.showerror('Set & Catch Database Is Empty', "Please Import Set & Catch CSV File To View")
        except sqlite3.Error as error:
            print('Error occured - ', error)
    ...
    get_Unit_AreaQC_df = get_Unit_AreaQC_dfDB()
    entry_Flagged.insert(tk.END,get_Unit_AreaQC_df[2])
    ObserverSetCatchDB = get_Unit_AreaQC_df[0]
    length_ObserverSetCatchDB = len(ObserverSetCatchDB)
    if length_ObserverSetCatchDB > 0:
        ObserverSetCatchDB = (ObserverSetCatchDB.loc[:,
            ['DeploymentUID', 'UAValidityCheck_StartPoints', 'UAValidityCheck_EndPoints',
            'StartPoints','EndPoints', 'Assigned_UnitAreaPolygon','Calculated_UnitAreaPolygon',
            'ASOCCode', 'ObserverNumber', 'Year', 'DeploymentNumber', 'SubTripNumber',
            'SetNumber', 'Country', 'Quota', 'SetType', 'VesselSideNumber',
            'VesselClass', 'VesselLength', 'VesselHorsepower', 'Day', 'Month', 
            'HaulDay','HaulMonth', 'StartTime', 'Duration', 'PositionPrecision',
        ...
        ObserverSetCatchDB[['UAValidityCheck_StartPoints', 'UAValidityCheck_EndPoints',
            'StartPoints','EndPoints', 'Assigned_UnitAreaPolygon', 'Calculated_UnitAreaPolygon', 
    ...
    def BackendSubmitAndUpdateDB(Complete_df):
        ...
        cur_DB_SetCatch_CalculationVal.executemany("UPDATE SetCatch_UnitAreaQCAnalysis SET \
                NAFODivision =?,  UnitArea =?, StartLatitude =? , StartLongitude = ?, EndLatitude =?, EndLongitude =?\
				WHERE DeploymentUID =?", 
				UpdateRecordList2_SetCatchDB)
    ...
    def SetCatchColName():
        SetCatchColNameList =  ['DeploymentUID', 'UAValidityCheck_StartPoints', 'UAValidityCheck_EndPoints',
        'StartPoints','EndPoints', 'Assigned_UnitAreaPolygon','Calculated_UnitAreaPolygon',
        'ASOCCode', 'ObserverNumber', 'Year', 'DeploymentNumber', 'SubTripNumber',
        'SetNumber', 'Country', 'Quota', 'SetType', 'VesselSideNumber',
        'VesselClass', 'VesselLength', 'VesselHorsepower', 'Day', 'Month', 
    ...
    def ReSetIndexDF_BackEnd():
        ...
        Complete_df = Complete_df.loc[:,['DeploymentUID', 'UAValidityCheck_StartPoints', 'UAValidityCheck_EndPoints',
        'StartPoints','EndPoints', 'Assigned_UnitAreaPolygon','Calculated_UnitAreaPolygon',
        'ASOCCode', 'ObserverNumber', 'Year', 'DeploymentNumber', 'SubTripNumber',
        'SetNumber', 'Country', 'Quota', 'SetType', 'VesselSideNumber',
        'VesselClass', 'VesselLength', 'VesselHorsepower', 'Day', 'Month', 
    ...
    def SetCatchDFQC(Complete_df):
        ...
        ObserverSetCatchDB[['UAValidityCheck_StartPoints', 'UAValidityCheck_EndPoints',
            'StartPoints','EndPoints', 'Assigned_UnitAreaPolygon', 'Calculated_UnitAreaPolygon','ObserverNumber', 'DeploymentUID', 
            'SubTripNumber','NAFODivision', 'VesselSideNumber',
            'UnitArea','DetailedCatchSpeciesCompCode']] = ObserverSetCatchDB[
            ['UAValidityCheck_StartPoints', 'UAValidityCheck_EndPoints',
            'StartPoints','EndPoints', 'Assigned_UnitAreaPolygon','Calculated_UnitAreaPolygon','ObserverNumber', 'DeploymentUID', 
    ...
    def DefaultColChooser():
        pt.prodyutCustomclearTable()
        Return_ObserverSetCatchDB = get_Unit_AreaQC_dfDB()
        ...
        ObserverSetCatchDB[['UAValidityCheck_StartPoints', 'UAValidityCheck_EndPoints',
            'StartPoints','EndPoints', 'Assigned_UnitAreaPolygon', 'ObserverNumber', 'DeploymentUID', 
            'SubTripNumber','NAFODivision', 'VesselSideNumber',
            'UnitArea','DetailedCatchSpeciesCompCode']] = ObserverSetCatchDB[
            ['UAValidityCheck_StartPoints', 'UAValidityCheck_EndPoints',
            'StartPoints','EndPoints', 'Assigned_UnitAreaPolygon','ObserverNumber', 'DeploymentUID', 
        ...
        ObserverSetCatchDB = (ObserverSetCatchDB.loc[:,
            ...
            'EndLatitude', 'EndLongitude','EndPoints', 'NAFODivision','Assigned_UnitAreaPolygon',
    ...
    def get_AllUnit_AreaQC_dfDB():
        ...
        try:
            ...
            if length_Unit_AreaQC_df > 0:
                Unit_AreaQC_df['Assigned_UnitAreaPolygon'] = (Unit_AreaQC_df
                ['Assigned_UnitAreaPolygon'].map(lambda x: str(x)[0:7])) + ' - ' +\
                 Unit_AreaQC_df['NAFODivision']
                Unit_AreaQC_df = Unit_AreaQC_df.loc[:,
                ['DeploymentUID', 'UAValidityCheck_StartPoints', 'UAValidityCheck_EndPoints',
                 'StartPoints','EndPoints', 'Assigned_UnitAreaPolygon', 'Calculated_UnitAreaPolygon', 'RecordType']]
                Unit_AreaQC_df = Unit_AreaQC_df.reset_index(drop=True)
                Unit_AreaQC_dfDB = pd.DataFrame(Unit_AreaQC_df)
                Unit_AreaQC_dfDB=  Unit_AreaQC_dfDB.merge(
                    Return_ObsSetCatchDB, 
                    ...
                    how='left').query('_merge == "both"')
                sqliteConnection.commit()
                return Unit_AreaQC_dfDB, length_Unit_AreaQC_df
            else:
                messagebox.showerror('Set & Catch Database Is Empty', "Please Import Set & Catch CSV File To View")
        except sqlite3.Error as error:
            print('Error occured - ', error)
    ...
    def GenQCFailAnalysisReport():
        ...
        Return_ObserverSetCatchDB = get_AllUnit_AreaQC_dfDB()
        ObserverSetCatchDB = Return_ObserverSetCatchDB[0]
        ObserverSetCatchDB = SetCatchDFQC(ObserverSetCatchDB)
        ObserverSetCatchDB = (ObserverSetCatchDB.loc[:,
        ['DeploymentUID', 'UAValidityCheck_StartPoints', 
    ...
    def viewSelCols_SetAndCatchDB(SelectedColumns):
        ...
        BaseColumns =['DeploymentUID', 'UAValidityCheck_StartPoints', 
                      'UAValidityCheck_EndPoints','StartPoints',
                      'EndPoints', 'Assigned_UnitAreaPolygon', 'Calculated_UnitAreaPolygon']
