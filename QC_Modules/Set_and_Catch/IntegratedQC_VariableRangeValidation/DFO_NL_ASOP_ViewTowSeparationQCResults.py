...
def ViewRangeValidatedResult_TowSeparation():
    windows = tk.Toplevel()
    windows.title ("DFO-NL-ASOP Range Validator - ID-D-02-1")
    windows.geometry('1540x855+40+40')
    ...
    lbl_SortLabel = Label(HeaderFrame, font=('aerial', 10, 'bold'),
            bg= "cadet blue", text="Tow Separation QC Table : ")
    ...
    lbl_TotalFlagged = Label(HeaderFrame, font=('aerial', 10, 'bold'),
            bg= "cadet blue", text="Flagged Entries : ")
    lbl_TotalFlagged.grid(row =0, column = 4, columnspan=1 ,padx=1, pady =2, sticky =W)

    EntryFlagged       = IntVar(HeaderFrame, value ='')
    entry_Flagged = Entry(HeaderFrame, font=('aerial', 10), justify='center',
                                textvariable = EntryFlagged, width = 5, bd=2)
    entry_Flagged.grid(row =0, column = 5, padx=2, pady =2, ipady =1, sticky =E)

    lbl_TotalEntries = Label(HeaderFrame, font=('aerial', 10, 'bold'),
            bg= "cadet blue", text="Total DB Entries (Set) : ")
    lbl_TotalEntries.grid(row =0, column = 9, columnspan=1 ,padx=1, pady =2, sticky =W)

    EntryTotal       = IntVar(HeaderFrame, value ='')
    entry_Total = Entry(HeaderFrame, font=('aerial', 10), justify='center',
                                textvariable = EntryTotal, width = 5, bd=2)
    entry_Total.grid(row =0, column = 10, padx=2, pady =2, ipady =1, sticky =W)
    ...
    def DeleteTempStorage():
        ...
        Complete_df = pd.read_sql_query("SELECT * FROM SetCatch_QCFailedRange_TempTowSeparation;", sqliteConnection)
        length_Complete_df = len(Complete_df)
        if length_Complete_df > 0:
            cursor.execute("DELETE FROM SetCatch_QCFailedRange_TempTowSeparation")
    ...
    def get_ObserverSetCatchDB():
        try:
            sqliteConnection = sqlite3.connect(DB_SetCatch_Validation_Range)
            cursor = sqliteConnection.cursor()
            Complete_df = pd.read_sql_query("SELECT * FROM SetCatch_QCFailedRange_TowSeparation;", sqliteConnection)
            length_Complete_df = len(Complete_df)
            if length_Complete_df > 0:
                Complete_df = Complete_df.reset_index(drop=True)
                ObserverSetCatchDB = pd.DataFrame(Complete_df)
                FlaggedCountDF = ObserverSetCatchDB[(ObserverSetCatchDB.TowSepErrorFlag) == 'Yes']
                lengthFlagged = len(FlaggedCountDF)
                sqliteConnection.commit()
    ...
    def BackendSubmitAndUpdateDB(Complete_df):
        ...
        cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET \
                ASOCCode =? , ObserverNumber = ?, Year =?, DeploymentNumber =?, SubTripNumber =?,\
                SetNumber =? , Country = ?, Quota =?, SetType =?, VesselSideNumber =?,\
                VesselClass =? , VesselLength = ?, VesselHorsepower =?, Day =?, Month =?,\
                HaulDay =? , HaulMonth = ?, StartTime =?, Duration =?, PositionPrecision =?,\
    ...
    def SetCatchDFQC(Complete_df):
        ...
        Complete_df[['HourSeparation', 'StartLatDec', 'StartLongDec', 'EndLatDec', 
                     'EndLongDec','TowSeparation', 'MaxTowSeparation', 
                     'Duration','StartLatitude','StartLongitude', 'EndLatitude', 
                     'EndLongitude','AverageTowSpeed', 'VesselLength']] = Complete_df[
                    ['HourSeparation', 'StartLatDec', 'StartLongDec', 'EndLatDec', 
                     'EndLongDec','TowSeparation', 'MaxTowSeparation', 
                     'Duration','StartLatitude','StartLongitude', 'EndLatitude', 
                     'EndLongitude','AverageTowSpeed', 'VesselLength']].astype(float)
        ...
        Complete_df[['TowSepErrorFlag', 'DepChangeFlag', 'SubTripChangeFlag',
                     'DeploymentUID','SubTripNumber','VesselSideNumber']
                     ] = Complete_df[['TowSepErrorFlag', 'DepChangeFlag', 'SubTripChangeFlag',
                     'DeploymentUID','SubTripNumber','VesselSideNumber']].astype(str)
    ...
    def DefaultSetIndexDF():
        ...
        button_ViewAllFlagged.config(state="normal")
        button_ViewCleartable.config(state="disabled")
        button_ViewAllFlagged.config(state="disabled")
   ...
   ## CV - Sel & Move Col - Section B
    ...
    def DefaultColChooser():
        ...
        ObserverSetCatchDB = Return_ObserverSetCatchDB[0]
        ObserverSetCatchDB = ObserverSetCatchDB.loc[:,
                ['DeploymentUID','StartDate','EndDate', 'HourSeparation', 'StartLatDec',
                 'StartLongDec', 'EndLatDec', 'EndLongDec','TowSeparation', 'MaxTowSeparation',
                 'TowSepErrorFlag', 'DepChangeFlag', 'SubTripChangeFlag',
    ...
    def ViewTowSeplaggedOnly():
        ...
        ObserverSetCatchDB = ObserverSetCatchDB.loc[:,
            ...
            'LogbookIDNumber2', 'SpeciesCode', 'KeptWeight', 'DiscardWeight', 'EstimatedWeightReleasedCrab', 
            'NumberIndividuals']]
        ObserverSetCatchDB = ObserverSetCatchDB[(ObserverSetCatchDB.TowSepErrorFlag) == 'Yes']
        ObserverSetCatchDB.sort_values(by=['Year', 'ASOCCode', 
            'DeploymentNumber', 'SetNumber'], ascending=True, inplace=True)
        ObserverSetCatchDB  = ObserverSetCatchDB.reset_index(drop=True)
        ObserverSetCatchDB = pd.DataFrame(ObserverSetCatchDB, index=None)
        ...
        ProcedureFrame = Frame(root, width = 40,  bd = 2,relief = RIDGE, bg= "cadet blue")
        ...
        ## Frame Of Flagging: Criteria Flagging
        FlaggingCriteriaframe = tk.Frame(root, bd = 2,relief = RIDGE, bg= "cadet blue")
        FlaggingCriteriaframe.pack(side =LEFT, padx=5, pady =2)

        lbl_Criteria_One = Label(FlaggingCriteriaframe, font=('aerial', 11, 'bold'),
                                bg= "cadet blue", text="B: Flagging Criteria")
        lbl_Criteria_One.grid(row =0, column = 0, columnspan=1 ,padx=2, pady =2, sticky =W)

        lbl_Criteria_Two = Label(FlaggingCriteriaframe, font=('aerial', 9, 'bold'),
                                bg= "cadet blue", text="1 : TowSepErrorFlag Must Be >> Yes")
        lbl_Criteria_Two.grid(row =2, column = 0, columnspan=1 ,padx=2, pady =2, sticky =W)

        lbl_Criteria_Three = Label(FlaggingCriteriaframe, font=('aerial', 9, 'bold'),
                                bg= "cadet blue", text="2 : DepChangeFlag & SubTripChangeFlag Must Be >> No")
        lbl_Criteria_Three.grid(row =3, column = 0, columnspan=1 ,padx=2, pady =2, sticky =W)

    def get_ViewAlltowflagged():
        try:
            sqliteConnection = sqlite3.connect(DB_SetCatch_Validation_Range)
            cursor = sqliteConnection.cursor()
            Complete_df = pd.read_sql_query("SELECT * FROM SetCatch_QCFailedRange_TowSeparation;", sqliteConnection)
    ...
    def ReloadViewAllTowSepEntries():
        Return_ObserverSetCatchDB = get_ViewAlltowflagged()
        ObserverSetCatchDB = Return_ObserverSetCatchDB[0]
    ...
    def viewSelCols_SetAndCatchDB(SelectedColumns):
        GetSetCatchDB_Columns = [
            ...
            'LogbookIDNumber2', 'SpeciesCode', 'KeptWeight', 'DiscardWeight', 'EstimatedWeightReleasedCrab', 
            'NumberIndividuals'] 
        BaseColumns =['DeploymentUID','StartDate','EndDate', 'HourSeparation', 
                      'StartLatDec', 'StartLongDec', 'EndLatDec', 'EndLongDec',
	                  'TowSeparation', 'MaxTowSeparation', 'TowSepErrorFlag', 
                      'DepChangeFlag', 'SubTripChangeFlag']
        TreeViewSelectedCols = list(BaseColumns + list((SelectedColumns)))
        con= sqlite3.connect(DB_SetCatch_Validation_Range)
        cur=con.cursor()
        cur.execute("SELECT * FROM SetCatch_QCFailedRange_TowSeparation")
        rows=cur.fetchall()
    ...
    ## CV - Alternate Procedure Edit/Update- Section E
    ...
    def AddForSubmitToUpdateDB(ObserverSetCatchDB):
        ...
        if len(Complete_df) >0:
            ...
            Complete_df = Complete_df.loc[:,
                ['StartDate','EndDate', 'HourSeparation', 'StartLatDec', 'StartLongDec', 'EndLatDec', 'EndLongDec',
                'TowSeparation', 'MaxTowSeparation', 'TowSepErrorFlag', 'DepChangeFlag', 
                'SubTripChangeFlag','DataBase_ID','RecordIdentifier','DeploymentUID',
                'ASOCCode', 'ObserverNumber', 'Year', 'DeploymentNumber', 'SubTripNumber',
                'SetNumber', 'Country', 'Quota', 'SetType', 'VesselSideNumber',
                'VesselClass', 'VesselLength', 'VesselHorsepower', 'Day', 'Month', 
                'HaulDay','HaulMonth', 'StartTime', 'Duration', 'PositionPrecision',
    ...
    def BackendAddForSubmitToUpdateDB(Complete_df):
        try:
            ...
            sqliteConnection = sqlite3.connect(DB_SetCatch_Validation_Range)
            cursor = sqliteConnection.cursor()
            Submit_To_DBStorage.to_sql('SetCatch_QCFailedRange_TempTowSeparation', sqliteConnection, 
...
### Top File Menu : Misc
    ...
    def SelectForUpdateEntry():
        SetCatchColNameList =  ['StartDate','EndDate', 'HourSeparation', 'StartLatDec', 'StartLongDec', 'EndLatDec', 'EndLongDec',
        'TowSeparation', 'MaxTowSeparation', 'TowSepErrorFlag', 'DepChangeFlag', 'SubTripChangeFlag',
        'DataBase_ID','RecordIdentifier','DeploymentUID',
        'ASOCCode', 'ObserverNumber', 'Year', 'DeploymentNumber', 'SubTripNumber',
        'SetNumber', 'Country', 'Quota', 'SetType', 'VesselSideNumber',
    ...
    def Lookup_User_DB_Variable_UpdateDB(get_Updated_Variable, UpdateRecordList):
        ...
        if get_Updated_Variable not in SetNoUpdateList:
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[0]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET ASOCCode = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[1]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET ObserverNumber = ? WHERE DeploymentUID =?", 
                    UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[2]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET Year = ? WHERE DeploymentUID =?", 
                        UpdateSetList)

            if get_Updated_Variable == GetSetCatchDB_VariableList[3]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET DeploymentNumber = ? WHERE DeploymentUID =?", 
                        (UpdateSetList))
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[4]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET SubTripNumber = ? WHERE DeploymentUID =?", 
                        (UpdateSetList))
            
            if get_Updated_Variable == GetSetCatchDB_VariableList[5]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET SetNumber = ? WHERE DeploymentUID =?", 
                        (UpdateSetList))
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[6]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET Country = ? WHERE DeploymentUID =?", 
                        (UpdateSetList))
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[7]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET Quota = ? WHERE DeploymentUID =?", 
                        (UpdateSetList))
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[8]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET SetType = ? WHERE DeploymentUID =?", 
                        (UpdateSetList))
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[9]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET VesselSideNumber = ? WHERE DeploymentUID =?", 
                        (UpdateSetList))
            
            if get_Updated_Variable == GetSetCatchDB_VariableList[10]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET VesselClass = ? WHERE DeploymentUID =?", 
                        (UpdateSetList))
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[11]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET VesselLength = ? WHERE DeploymentUID =?", 
                        (UpdateSetList))

            if get_Updated_Variable == GetSetCatchDB_VariableList[12]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET VesselHorsepower = ? WHERE DeploymentUID =?", 
                        (UpdateSetList))
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[13]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET Day = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[14]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET Month = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[15]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET HaulDay = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[16]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET HaulMonth = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[17]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET StartTime = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[18]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET Duration = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[19]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET PositionPrecision = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[20]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET StartLatitude = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[21]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET StartLongitude = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[22]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET EndLatitude = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[23]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET EndLongitude = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[24]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET NAFODivision = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[25]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET UnitArea = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
            
            if get_Updated_Variable == GetSetCatchDB_VariableList[26]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET StatisticalArea = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[27]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET InOut200MileLimit = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[28]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET GearType = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[29]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET CodendMeshSize = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[30]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET MeshSizeMG = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[31]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET MeshSizeFG = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[32]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET RollerBobbbinDiameter = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[33]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET NumberGillnets = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[34]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET AverageGillnetLength = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[35]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET GrateBarSpacing = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[36]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET FootropeLength = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[37]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET NumberWindows = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[38]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET NumberHooks = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[39]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET NumberPots = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[40]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET NumberPotReleasedCrab = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[41]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET GearDamage = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[42]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET AverageTowSpeed = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[43]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET AverageDepth = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[44]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET DataSource = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                
            if get_Updated_Variable == GetSetCatchDB_VariableList[45]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET DirectedSpecies = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[46]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET NumberSpecies = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[47]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET RecordType = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[48]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET DetailedCatchSpeciesCompCode = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
            
            if get_Updated_Variable == GetSetCatchDB_VariableList[49]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET LogbookIDNumber1 = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
                    
            if get_Updated_Variable == GetSetCatchDB_VariableList[50]:
                cur_DB_SetCatch_Validation_Range.executemany("UPDATE SetCatch_QCFailedRange_TowSeparation SET LogbookIDNumber2 = ? WHERE DeploymentUID =?", 
                        UpdateSetList)
   ...
   ## Bottom Left Frame- ProcedureFrame Define
    ...
    lbl_QCEditDBEntry = Label(QCEditDBEntry, font=('aerial', 11, 'bold'),
                            bg= "cadet blue", text="F: Alternate Procedure - 2")
    ...
    ## Header Frame
    button_ReloadView = Button(HeaderFrame, bd = 2, text ="↓ Ascending Sort ↓ ", width =17,
                                height=1, font=('aerial', 10, 'bold'), fg="blue", bg="cadet blue", 
                                command =ViewAssendingSortVariable)
    button_ReloadView.grid(row =0, column = 2, columnspan=1 ,padx=2, pady =2, sticky =W)

    button_SortHighToLow = Button(HeaderFrame, bd = 2, text ="↑ Descending Sort ↑ ", width =17,
                                height=1, font=('aerial', 10, 'bold'), fg="blue", bg="cadet blue", 
                                command =ViewDesendingSortVariable)
    button_SortHighToLow.grid(row =0, column = 3, columnspan=1 ,padx=2, pady =2, sticky =W)

    button_ViewAllFlagged = Button(HeaderFrame, bd = 2, text ="Reload & View All Entries", width =23,
                                height=1, font=('aerial', 10, 'bold'), fg="blue", bg="cadet blue", 
                                command =ReloadViewAllTowSepEntries)
    button_ViewAllFlagged.grid(row =0, column = 6, padx=12, pady =1, ipady =1, sticky =W)

    button_ViewCleartable = Button(HeaderFrame, bd = 2, text ="Clear View", width =9,
                                height=1, font=('aerial', 10, 'bold'), fg="blue", bg="cadet blue", 
                                command =ViewClearView)
    button_ViewCleartable.grid(row =0, column = 7, padx=4, pady =1, ipady =1, sticky =W)

    button_ReloadViewFlagged = Button(HeaderFrame, bd = 2, text ="View Flagged Entries Only", width =23,
                                height=1, font=('aerial', 10, 'bold'), fg="blue", bg="cadet blue", 
                                command =ViewTowSeplaggedOnly)
    button_ReloadViewFlagged.grid(row =0, column = 8, padx=4, pady =1, ipady =1, sticky =W)
    ...
    View.add_command(label="View Flagged Entries Only", command=ViewTowSeplaggedOnly)
