...
def SetCatch_ViewLookupValidatedResult_DirectedSpecies():
    window = tk.Tk()
    window.title ("DFO-NL-ASOP Lookup DirectedSpecies Codes Validator - ID-C-01-10")
    ...
    DirectedSpecies   = IntVar(TopLeftframe, value='')

    lblTitEntry = Label(TopLeftframe, font=('aerial', 12, 'bold'), text=" A: DirectedSpecies QC Failed Profile Details :")
    ...
    lblDirectedSpeciesCode = Label(TopLeftframe, font=('aerial', 10, 'bold'), text = "4. DirectedSpecies :", padx =0, pady= 2)
    lblDirectedSpeciesCode.grid(row =8, column = 0, padx=4, pady =2, sticky =W)
    txtDirectedSpeciesCode  = Entry(TopLeftframe, font=('aerial', 12, 'bold'),textvariable= DirectedSpecies, width = 35)
    txtDirectedSpeciesCode.grid(row =9, column = 0, padx=4, pady =4,ipadx=1, sticky =W)
    ...
    tree1.heading("#4", text="DirectedSpecies", anchor=CENTER)
    ...
    tree2.heading("#2", text="DirectedSpecies", anchor=CENTER)
    ...
    def GetDirectedSpeciesProfileDB():
        try:
            ...
            if len(Complete_df) >0:
                ...
                SpeciesCodeProfileDB_DF.rename(columns={"SpeciesCode":"DirectedSpecies"},inplace = True)
                SpeciesCodeProfileDB_DF = pd.DataFrame(SpeciesCodeProfileDB_DF)
                return SpeciesCodeProfileDB_DF
            else:
                messagebox.showerror('DirectedSpecies Code Lookup Table Message', 
                                    "Void DirectedSpecies Code Lookup Table...Please Import/Insert DirectedSpecies Coded Lookup Table")
    ...
    def viewQCFailed():
        try:
            ...
            cur=con.cursor()
            cur.execute("SELECT DataBase_ID, RecordIdentifier, DeploymentUID, DirectedSpecies FROM SetCatch_QCFailedLookUpTable_DirectedSpecies ORDER BY `DataBase_ID` ASC")
            rows=cur.fetchall()
            return rows
        except sqlite3.Error as error:
            print('Error occured - ', error)
    ...
    def viewQCFailed_DirectedSpeciesProfile():
        ClearDirectedSpeciesFailedResultsTable_A()
        countIndex = 0
        rows = viewQCFailed()
        for row in rows:
            if countIndex % 2 == 0:
    ...
    def QCFailedTotalEntries():
        ...
        Complete_df= pd.read_sql_query("SELECT * FROM SetCatch_QCFailedLookUpTable_DirectedSpecies ORDER BY `DataBase_ID` ASC;", conn)
    ...
    def iExit():
        iExit= tkinter.messagebox.askyesno("DFO-NL-ASOP DirectedSpecies Codes Profile", "Confirm If You Want To Exit")
        if iExit >0:
            window.destroy()
            return

    def ClearQCFailedDirectedSpeciesDetails():
        txtDatabaseID.delete(0,END)
        txtRecordIdentifier.delete(0,END)
        txtDeploymentUID.delete(0,END)
        txtDirectedSpeciesCode.delete(0,END)
        
    def CheckDirectedSpeciesCodeValidity(UserEntryDirectedSpeciesCode):
        DirectedSpeciesCodeFound = "Modied/Updated DirectedSpecies Code Found In DirectedSpecies lookup Table"
        DirectedSpeciesCodeNotFound = "Modied/Updated DirectedSpecies Code Not Found In DirectedSpecies lookup Table"
        if((UserEntryDirectedSpeciesCode!='')):
            try:
                UserEntryDirectedSpeciesCode = int(UserEntryDirectedSpeciesCode)
                Complete_df = GetDirectedSpeciesProfileDB()
                data = pd.DataFrame(Complete_df)
                data = data.reset_index(drop=True)
                if (UserEntryDirectedSpeciesCode in data["DirectedSpecies"].unique()) == True: 
                    return DirectedSpeciesCodeFound
                else: 
                    return DirectedSpeciesCodeNotFound
            except:
                return tkinter.messagebox.showerror("Update Error"," DirectedSpecies Must Be Integer Value")
        else:
                return tkinter.messagebox.showerror("Update Error"," DirectedSpecies Code Entry Can Not Be Empty")
        
    def Modify_SetCatch_DirectedSpeciesProfile():
        DirectedSpeciesCodeFound = "Modied/Updated DirectedSpecies Code Found In DirectedSpecies lookup Table"
        DirectedSpeciesCodeNotFound = "Modied/Updated DirectedSpecies Code Not Found In DirectedSpecies lookup Table"
        UserEntryDirectedSpeciesCode = txtDirectedSpeciesCode.get()
        EntryValidityDirectedSpeciesCode = CheckDirectedSpeciesCodeValidity(UserEntryDirectedSpeciesCode)
        if EntryValidityDirectedSpeciesCode == DirectedSpeciesCodeFound:
            cur_id = tree1.focus()
            ...
            if Length_Selected != 0:
                ...
                if (len(SelectionTree)>0) & (len(SelectionTree)==1) & (len(SelectionTree)<2):
                    ...
                    if iUpdate >0:
                        ...
                        if (list_item_DatabaseUID == DataBase_ID) & \
                            ...
                            if(DirectedSpecies!=0):
                                ...
                                cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET DirectedSpecies = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (DirectedSpecies, DataBase_ID, RecordIdentifier, DeploymentUID))
                                cur_DB_SetCatch_Validation_LookupTables.execute("UPDATE SetCatch_QCFailedLookUpTable_DirectedSpecies SET DirectedSpecies = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (DirectedSpecies, DataBase_ID, RecordIdentifier, DeploymentUID))
                                ...
                                tree1.insert("", tk.END,values=(DataBase_ID, RecordIdentifier, DeploymentUID, DirectedSpecies ))
                                QCFailedTotalEntries()
                                ClearDirectedSpeciesFailedSummaryTable_B()
                            else:
                                tkinter.messagebox.showerror("Update Error"," DirectedSpecies Code Entry can not be empty")
                        else:
                    ...
                    UpdatePossibleError =  ("Possible Reasons For Update Error: " + '\n' + '\n' + 
                            "1. Please Select One Entry Only From The DirectedSpecies Table To Update "  + '\n' + '\n' + 
                            "2. Please Donot Select More Than One Entry To Update At The Same time "  + '\n' + '\n' + 
                            "3. Please Do Not Update Without Selecting Entry From DirectedSpecies Table" )
            ...
            tkinter.messagebox.showerror("Update/Modification Error In Set & Catch DB", DirectedSpeciesCodeNotFound + ".  " +  "Please Update DirectedSpecies Code Lookup Table With The Correct DirectedSpecies Code")

    def ClearAllDirectedSpeciesQCFailedTable():
        tree1.delete(*tree1.get_children())
    ...
    def InventoryRec1(event):
        for nm in tree1.selection():
            ...
            txtDirectedSpeciesCode.delete(0,END)
            txtDirectedSpeciesCode.insert(tk.END,sd[3])

    def Export_QCFailedDirectedSpecies_CSV():
        try:
            conn = sqlite3.connect(DB_SetCatch_Validation_LookUpTable)
            cursor = conn.cursor()
            Complete_df = pd.read_sql_query("SELECT * FROM SetCatch_QCFailedLookUpTable_DirectedSpecies ORDER BY `DataBase_ID` ASC ;", conn)
            if len(Complete_df) >0:
                ...
                if len(filename) >0:
                    ...
                    tkinter.messagebox.showinfo("QC Failed DirectedSpecies Profile","QC Failed DirectedSpecies Profile Report Saved as CSV")
                else:
                    tkinter.messagebox.showinfo("QC Failed DirectedSpecies Profile Report Message","Please Select File Name To Export")
            else:
    ...
    def Modify_MultipleSetCatch_DirectedSpeciesProfile():
        dfList =[] 
            ...
            if Length_Selected != 0:
                ...
                if (len(SelectionTree)>0):
                    iUpdate = tkinter.messagebox.askyesno("Update Multiple Entries In  DFO-NL-ASOP Set & Catch Database", 
                            "Confirm If You Want To Update Multiple DirectedSpecies Code Entries In DFO-NL-ASOP Set & Catch Database")
                    if iUpdate >0:
                        application_window = window
                        Input_UpdateAnswer = simpledialog.askstring("Input Updated DirectedSpecies Code", "What is Your Updated DirectedSpecies Code Value?",
                                                                parent=application_window)
                        if Input_UpdateAnswer is not None:
                            Input_UpdatedDirectedSpeciesCode = (Input_UpdateAnswer)
                            DirectedSpeciesCodeFound = "Modied/Updated DirectedSpecies Code Found In DirectedSpecies lookup Table"
                            DirectedSpeciesCodeNotFound = "Modied/Updated DirectedSpecies Code Not Found In DirectedSpecies lookup Table"
                            EntryValidityDirectedSpeciesCode = CheckDirectedSpeciesCodeValidity(Input_UpdatedDirectedSpeciesCode)
                            if EntryValidityDirectedSpeciesCode == DirectedSpeciesCodeFound:
                                conn_DB_Set_Catch_Analysis= sqlite3.connect(DB_Set_Catch_Analysis)
                                cur_DB_Set_Catch_Analysis=conn_DB_Set_Catch_Analysis.cursor()
                                conn_DB_SetCatch_Validation_LookupTables= sqlite3.connect(DB_SetCatch_Validation_LookUpTable)
                                ...
                                for item in SelectionTree:
                                    ...
                                    list_item_RecordIdentifier = int(list_item[1])
                                    list_item_DeploymentUID = (list_item[2])
                                    Input_UpdatedDirectedSpeciesCode =int(Input_UpdatedDirectedSpeciesCode)
                                    cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET DirectedSpecies = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (Input_UpdatedDirectedSpeciesCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                    cur_DB_SetCatch_Validation_LookupTables.execute("UPDATE SetCatch_QCFailedLookUpTable_DirectedSpecies SET DirectedSpecies = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                        (Input_UpdatedDirectedSpeciesCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                conn_DB_Set_Catch_Analysis.commit()
                                conn_DB_Set_Catch_Analysis.close()
                                conn_DB_SetCatch_Validation_LookupTables.commit()
                                conn_DB_SetCatch_Validation_LookupTables.close()
                                viewQCFailed_DirectedSpeciesProfile()
                                ClearDirectedSpeciesFailedSummaryTable_B()
                                return
                            else:
                                tkinter.messagebox.showerror("Update/Modification Error In Set & Catch DB",
                                                            DirectedSpeciesCodeNotFound + ".  " +  
                                                            "Please Update DirectedSpecies Code Lookup Table With The Correct DirectedSpecies Code")
                        else:
                            tkinter.messagebox.showinfo("Update Error","Please Input Updated DirectedSpecies Code Value")   
            else:
                tkinter.messagebox.showinfo("Update Error","Please Select At least One Entries To Update DirectedSpecies Code")
        else:
            tkinter.messagebox.showinfo("Update Error","Empty DirectedSpecies Codes Table. Please Select At least One Entries In The Table To Update DirectedSpecies Code")
    ...
    def GetSetCatchQCFailedDB():
        try:
            ...
            if len(Complete_df) >0:
                ...
                SetCatchQCFailedDB_DF = SetCatchQCFailedDB_DF.loc[:,['DataBase_ID','RecordIdentifier','DeploymentUID','DirectedSpecies','DeploymentIdentifier']]
    ...
    def RefFailedToSetcatchDB():
        ...
        SetCatchQCFailedDB_DF['DirectedSpecies'] = (SetCatchQCFailedDB_DF.loc[:,['DirectedSpecies']]).astype(int, errors='ignore')

        Ref_FailedQC_InSetcatchDB = SetCatchProfileDB_DF.merge(
                                    SetCatchQCFailedDB_DF, on = ["DataBase_ID", "RecordIdentifier", "DeploymentUID", "DirectedSpecies"], indicator=True, 
        ...
        Ref_FailedQC_InSetcatchDB['DirectedSpecies'] = (Ref_FailedQC_InSetcatchDB.loc[:,['DirectedSpecies']]).astype(int, errors='ignore')
        Ref_FailedQC_InSetcatchDB['DirectedSpecies'] = (Ref_FailedQC_InSetcatchDB.loc[:,['DirectedSpecies']]).replace(99999999, '')
        ...
        root = tk.Tk()
        root.title ("DFO-NL-ASOP DirectedSpecies Codes Validation Failed On Set & Catch Imported Data")
    ...
    def SubmitDataFrameToQCFailedDB(DataFrameToSubmit):
        try:
            ...
            Import_To_DBStorage.to_sql('SetCatch_QCFailedLookUpTable_DirectedSpecies', sqliteConnection, if_exists="replace", index =False)
    ...
    def GenDepIdfier_Col_SetCatchFailDB():
        QCFailedDirectedSpeciesDB= GetSetCatchQCFailedDB()
        if len(QCFailedDirectedSpeciesDB)>0:
            txtTotalFailedEntries.delete(0,END)
            QCFailedDirectedSpeciesDB['DeploymentIdentifier'] = QCFailedDirectedSpeciesDB['DeploymentUID'].apply(lambda x: '-'.join(y[:-4] if i == len(x.split('-'))-1 else y for i,y in enumerate(x.split('-'))))
            QCFailedDirectedSpeciesDB['DeploymentIdentifier'] = QCFailedDirectedSpeciesDB['DeploymentIdentifier'].apply(lambda x: x[:-1] if x.endswith('-') else x)
            QCFailedDirectedSpeciesDB = QCFailedDirectedSpeciesDB.reset_index(drop=True)
            DataFrameToSubmit = pd.DataFrame(QCFailedDirectedSpeciesDB)
            SubmitDataFrameToQCFailedDB(DataFrameToSubmit)
            QCFailedTotalEntries = len(DataFrameToSubmit)       
            txtTotalFailedEntries.insert(tk.END,QCFailedTotalEntries)

    def ClearDirectedSpeciesFailedSummaryTable_B():
        tree2.delete(*tree2.get_children())
        txtSelectedFailedEntries.delete(0,END)

    def ClearDirectedSpeciesFailedResultsTable_A():
        tree1.delete(*tree1.get_children())
        txtTotalFailedEntries.delete(0,END)
    ...
    def GenQCFailedSummaryTable():
        ClearDirectedSpeciesFailedSummaryTable_B()
        QCFailedDirectedSpeciesDB= GetSetCatchQCFailedDB()
        QCFailedDirectedSpeciesDB['DirectedSpecies'] = (QCFailedDirectedSpeciesDB.loc[:,['DirectedSpecies']]).replace(99999999, '')
        QCFailedDirectedSpeciesDB['DirectedSpecies'] = (QCFailedDirectedSpeciesDB.loc[:,['DirectedSpecies']]).astype(int, errors='ignore')
        QCFailedDirectedSpeciesDB   = QCFailedDirectedSpeciesDB.groupby(['DeploymentIdentifier', 'DirectedSpecies'], as_index=False).DataBase_ID.count()
        QCFailedDirectedSpeciesDB   = pd.DataFrame(QCFailedDirectedSpeciesDB)
        QCFailedDirectedSpeciesDB.rename(columns={'DeploymentIdentifier':'DeploymentIdentifier',
                                            'DirectedSpecies': 'DirectedSpecies', 
        ...
        QCFailedDirectedSpeciesDB = pd.DataFrame(QCFailedDirectedSpeciesDB)
        countIndex1 = 0
        for each_rec in range(len(QCFailedDirectedSpeciesDB)):
            if countIndex1 % 2 == 0:
                tree2.insert("", tk.END, values=list(QCFailedDirectedSpeciesDB.loc[each_rec]), tags =("even",))
            else:
                tree2.insert("", tk.END, values=list(QCFailedDirectedSpeciesDB.loc[each_rec]), tags =("odd",))
    ...
    def Search_AND_MultiVariableQuery_Backend(DeploymentIdentifier ="", DirectedSpecies =""):
        try:
            ...
            cur_DB_SetCatch_Validation_LookUpTable.execute("SELECT * FROM SetCatch_QCFailedLookUpTable_DirectedSpecies WHERE \
                                            (DeploymentIdentifier = :DeploymentIdentifier OR TRIM(COALESCE(:DeploymentIdentifier, '')) = '') AND\
                                            (DirectedSpecies= :DirectedSpecies)",\
                                            (DeploymentIdentifier, DirectedSpecies))
            rows=cur_DB_SetCatch_Validation_LookUpTable.fetchall()
            return rows
        except:
            messagebox.showerror('DirectedSpecies Variable Error Message', "DirectedSpecies Query Failed")
    ...
    def InventoryRec2(event):
        for nm in tree2.selection():
            ...
            txtDirectedSpeciesCode.delete(0,END)
            txtDirectedSpeciesCode.insert(tk.END,sd[1])
            DeploymentIdentifier = sd[0]
            DirectedSpecies = sd[1]
            MultiSearchRows = Search_AND_MultiVariableQuery_Backend(DeploymentIdentifier, DirectedSpecies)
            MultiSearchRowsDF = pd.DataFrame(MultiSearchRows, columns =['DataBase_ID', 'RecordIdentifier', 'DeploymentUID','DirectedSpecies','DeploymentIdentifier'])
            MultiSearchRowsDF = MultiSearchRowsDF.loc[:,['DataBase_ID','RecordIdentifier','DeploymentUID','DirectedSpecies','DeploymentIdentifier']]
    ...
    def SummarySelectUpdateClearFromQCFailDB():
        ...
        if len(ListBox_DF)>0:
            ...
            if Length_Selected != 0:
                ...
                if (len(SelectionTree)>0) & (len(SelectionTree)==1) & (len(SelectionTree)<2):
                    application_window = window
                    Input_UpdateAnswer = simpledialog.askstring("Input Updated DirectedSpecies Code", "What is Your Updated DirectedSpecies Code Value?", parent=application_window)
                    if Input_UpdateAnswer is not None:
                        Input_UpdatedDirectedSpeciesCode = (Input_UpdateAnswer)
                        DirectedSpeciesCodeFound = "Modied/Updated DirectedSpecies Code Found In DirectedSpecies lookup Table"
                        DirectedSpeciesCodeNotFound = "Modied/Updated DirectedSpecies Code Not Found In DirectedSpecies lookup Table"
                        ...
                        if EntryValidityDirectedSpeciesCode == DirectedSpeciesCodeFound:
                            ...
                            for child in tree1.get_children():
                                ...
                                cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET DirectedSpecies = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (Input_UpdatedDirectedSpeciesCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                cur_DB_SetCatch_Validation_LookupTables.execute("DELETE FROM SetCatch_QCFailedLookUpTable_DirectedSpecies WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID = ? ", 
            ...
            tkinter.messagebox.showinfo("Update Error","Empty DirectedSpecies Summary Table : B")
    ...
    btnModifyDirectedSpeciesProfile = Button(TopLeftframe, text="Modify Code & Update DB", font=('aerial', 10, 'bold'), 
                            height =1, width=25, bd=2, command = Modify_SetCatch_DirectedSpeciesProfile)
    btnModifyDirectedSpeciesProfile.grid(row =10, column = 0, padx=2, pady =5, ipady =5, sticky =W)

    btnClearDetails = Button(TopLeftframe, text="Clear Details", font=('aerial', 10, 'bold'),
                            height =1, width=12, bd=1, command = ClearQCFailedDirectedSpeciesDetails)
    btnClearDetails.grid(row =10, column = 0, padx=2, pady =5, ipady =5, sticky =E)

    ## Button Wizard : Table B
    btnGenQCFailedDirectedSpeciesSummary = Button(TopBottomLeftframe, text="Generate Failed Summary", font=('aerial', 11, 'bold'), bg='alice blue',
                            height =1, width=21, bd=2, command = GenQCFailedSummaryTable)
    btnGenQCFailedDirectedSpeciesSummary.grid(row =2, column = 0, padx=2, pady =4, ipady =8, sticky =W)
    ...
    btnClearDirectedSpeciesTable = Button(TopRightframe, text="Clear All Table", font=('aerial', 11, 'bold'), 
                                height =1, width=12, bd=1, command = ClearAllDirectedSpeciesQCFailedTable)
    btnClearDirectedSpeciesTable.grid(row =2, column = 0, padx=2, pady =1, ipady =2, sticky =E)


    btnViewQCFailedQCDirectedSpeciesResults = Button(TopRightframe, text="View QC-Failed Results", font=('aerial', 11, 'bold'), bg='alice blue',
                            height =1, width=22, bd=2, command = viewQCFailed_DirectedSpeciesProfile)
    btnViewQCFailedQCDirectedSpeciesResults.grid(row =4, column = 0, padx=2, pady =2, ipady =4, sticky =W)
    ...
    filemenu.add_command(label="Export DB All QC Failed Results", command=Export_QCFailedDirectedSpecies_CSV)

    Treepopup.add_command(label="Modify & Update DB With Selected DirectedSpecies In Table C", command=Modify_MultipleSetCatch_DirectedSpeciesProfile)
