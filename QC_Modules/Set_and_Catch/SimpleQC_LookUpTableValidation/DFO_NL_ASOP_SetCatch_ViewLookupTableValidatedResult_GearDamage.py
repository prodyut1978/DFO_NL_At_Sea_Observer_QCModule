...
def SetCatch_ViewLookupValidatedResult_GearDamage():
    window = tk.Tk()
    window.title ("DFO-NL-ASOP Lookup GearDamage Codes Validator - ID-C-01-3")
    ...
    GearDamage   = IntVar(TopLeftframe, value='')

    lblTitEntry = Label(TopLeftframe, font=('aerial', 12, 'bold'), text=" A: GearDamage QC Failed Profile Details :")
    ...
    lblGearDamageCode = Label(TopLeftframe, font=('aerial', 10, 'bold'), text = "4. GearDamage :", padx =0, pady= 2)
    lblGearDamageCode.grid(row =8, column = 0, padx=4, pady =2, sticky =W)
    txtGearDamageCode  = Entry(TopLeftframe, font=('aerial', 12, 'bold'),textvariable= GearDamage, width = 35)
    txtGearDamageCode.grid(row =9, column = 0, padx=4, pady =4,ipadx=1, sticky =W)
    ...
    lblQCFailedResultGearDamage = Label(TopRightframe, font=('aerial', 11, 'bold'), text="C: View QC Failed Results Table")
    lblQCFailedResultGearDamage.grid(row =2, column = 0, padx=5, pady =2, ipady =2, sticky =W, rowspan =1)
    ...
    tree1.heading("#4", text="GearDamage", anchor=CENTER)
    ...
    tree2.heading("#2", text="GearDamage", anchor=CENTER)
    ...
    def GetGearDamageProfileDB():
        try:
            ...
            if len(Complete_df) >0:
                ...
                messagebox.showerror('GearDamage Code Lookup Table Message', 
                                    "Void GearDamage Code Lookup Table...Please Import/Insert GearDamage Coded Lookup Table")
    ...
    def viewQCFailed():
        try:
            ...
            cur=con.cursor()
            cur.execute("SELECT DataBase_ID, RecordIdentifier, DeploymentUID, GearDamage FROM SetCatch_QCFailedLookUpTable_GearDamage ORDER BY `DataBase_ID` ASC")
            rows=cur.fetchall()
            return rows
        except sqlite3.Error as error:
            print('Error occured - ', error)
    ...
    def viewQCFailed_GearDamageProfile():
        ClearGearDamageFailedResultsTable_A()
        countIndex = 0
        rows = viewQCFailed()
        for row in rows:
            if countIndex % 2 == 0:
    ...
    def QCFailedTotalEntries():
        ...
        Complete_df= pd.read_sql_query("SELECT * FROM SetCatch_QCFailedLookUpTable_GearDamage ORDER BY `DataBase_ID` ASC;", conn)
    ...
    def ClearQCFailedGearDamageDetails():
        txtDatabaseID.delete(0,END)
        txtRecordIdentifier.delete(0,END)
        txtDeploymentUID.delete(0,END)
        txtGearDamageCode.delete(0,END)
        
    def CheckGearDamageCodeValidity(UserEntryGearDamageCode):
        GearDamageCodeFound = "Modied/Updated GearDamage Code Found In GearDamage lookup Table"
        GearDamageCodeNotFound = "Modied/Updated GearDamage Code Not Found In GearDamage lookup Table"
        if((UserEntryGearDamageCode!='')):
            try:
                UserEntryGearDamageCode = int(UserEntryGearDamageCode)
                Complete_df = GetGearDamageProfileDB()
                data = pd.DataFrame(Complete_df)
                data = data.reset_index(drop=True)
                if (UserEntryGearDamageCode in data["GearDamage"].unique()) == True: 
                    return GearDamageCodeFound
                else: 
                    return GearDamageCodeNotFound
            except:
                return tkinter.messagebox.showerror("Update Error"," GearDamage Must Be Integer Value")
        else:
                return tkinter.messagebox.showerror("Update Error"," GearDamage Code Entry Can Not Be Empty")
        
    def Modify_SetCatch_GearDamageProfile():
        GearDamageCodeFound = "Modied/Updated GearDamage Code Found In GearDamage lookup Table"
        GearDamageCodeNotFound = "Modied/Updated GearDamage Code Not Found In GearDamage lookup Table"
        UserEntryGearDamageCode = txtGearDamageCode.get()
        EntryValidityGearDamageCode = CheckGearDamageCodeValidity(UserEntryGearDamageCode)
        if EntryValidityGearDamageCode == GearDamageCodeFound:
            cur_id = tree1.focus()
            ...
            if Length_Selected != 0:
                ...
                if (len(SelectionTree)>0) & (len(SelectionTree)==1) & (len(SelectionTree)<2):
                    ...
                    if iUpdate >0:
                        ...
                        GearDamage = txtGearDamageCode.get()
                        if (list_item_DatabaseUID == DataBase_ID) & \
                            ...
                            if(GearDamage!=0):
                                ...
                                cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET GearDamage = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (GearDamage, DataBase_ID, RecordIdentifier, DeploymentUID))
                                cur_DB_SetCatch_Validation_LookupTables.execute("UPDATE SetCatch_QCFailedLookUpTable_GearDamage SET GearDamage = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (GearDamage, DataBase_ID, RecordIdentifier, DeploymentUID))
                                ...
                                tree1.insert("", tk.END,values=(DataBase_ID, RecordIdentifier, DeploymentUID, GearDamage ))
                                QCFailedTotalEntries()
                                ClearGearDamageFailedSummaryTable_B()
                            else:
                                tkinter.messagebox.showerror("Update Error"," GearDamage Code Entry can not be empty")
                        else:
            ...
            tkinter.messagebox.showerror("Update/Modification Error In Set & Catch DB", GearDamageCodeNotFound + ".  " +  "Please Update GearDamage Code Lookup Table With The Correct GearDamage Code")

    def ClearAllGearDamageQCFailedTable():
        tree1.delete(*tree1.get_children())
    ...
    def InventoryRec1(event):
        for nm in tree1.selection():
            ...
            txtGearDamageCode.delete(0,END)
            txtGearDamageCode.insert(tk.END,sd[3])

    def Export_QCFailedGearDamage_CSV():
        try:
            conn = sqlite3.connect(DB_SetCatch_Validation_LookUpTable)
            cursor = conn.cursor()
            Complete_df = pd.read_sql_query("SELECT * FROM SetCatch_QCFailedLookUpTable_GearDamage ORDER BY `DataBase_ID` ASC ;", conn)
            if len(Complete_df) >0:
                ...
                if len(filename) >0:
                    ...
                    tkinter.messagebox.showinfo("QC Failed GearDamage Profile","QC Failed GearDamage Profile Report Saved as CSV")
                else:
                    tkinter.messagebox.showinfo("QC Failed GearDamage Profile Report Message","Please Select File Name To Export")
            else:
    ...
    def Modify_MultipleSetCatch_GearDamageProfile():
        dfList =[] 
            ...
            if Length_Selected != 0:
                ...
                if (len(SelectionTree)>0):
                    iUpdate = tkinter.messagebox.askyesno("Update Multiple Entries In  DFO-NL-ASOP Set & Catch Database", 
                            "Confirm If You Want To Update Multiple GearDamage Code Entries In DFO-NL-ASOP Set & Catch Database")
                    if iUpdate >0:
                        application_window = window
                        Input_UpdateAnswer = simpledialog.askstring("Input Updated GearDamage Code", "What is Your Updated GearDamage Code Value?",
                                                                parent=application_window)
                        if Input_UpdateAnswer is not None:
                            Input_UpdatedGearDamageCode = (Input_UpdateAnswer)
                            GearDamageCodeFound = "Modied/Updated GearDamage Code Found In GearDamage lookup Table"
                            GearDamageCodeNotFound = "Modied/Updated GearDamage Code Not Found In GearDamage lookup Table"
                            EntryValidityGearDamageCode = CheckGearDamageCodeValidity(Input_UpdatedGearDamageCode)
                            if EntryValidityGearDamageCode == GearDamageCodeFound:
                                conn_DB_Set_Catch_Analysis= sqlite3.connect(DB_Set_Catch_Analysis)
                                cur_DB_Set_Catch_Analysis=conn_DB_Set_Catch_Analysis.cursor()
                                conn_DB_SetCatch_Validation_LookupTables= sqlite3.connect(DB_SetCatch_Validation_LookUpTable)
                                ...
                                for item in SelectionTree:
                                    ...
                                    list_item_RecordIdentifier = int(list_item[1])
                                    list_item_DeploymentUID = (list_item[2])
                                    Input_UpdatedGearDamageCode =int(Input_UpdatedGearDamageCode)
                                    cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET GearDamage = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (Input_UpdatedGearDamageCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                    cur_DB_SetCatch_Validation_LookupTables.execute("UPDATE SetCatch_QCFailedLookUpTable_GearDamage SET GearDamage = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                        (Input_UpdatedGearDamageCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                conn_DB_Set_Catch_Analysis.commit()
                                conn_DB_Set_Catch_Analysis.close()
                                conn_DB_SetCatch_Validation_LookupTables.commit()
                                conn_DB_SetCatch_Validation_LookupTables.close()
                                viewQCFailed_GearDamageProfile()
                                ClearGearDamageFailedSummaryTable_B()
                                return
                            else:
                                tkinter.messagebox.showerror("Update/Modification Error In Set & Catch DB",
                                                            GearDamageCodeNotFound + ".  " +  
                                                            "Please Update GearDamage Code Lookup Table With The Correct GearDamage Code")
                        else:
                            tkinter.messagebox.showinfo("Update Error","Please Input Updated GearDamage Code Value")   
            else:
                tkinter.messagebox.showinfo("Update Error","Please Select At least One Entries To Update GearDamage Code")
        else:
            tkinter.messagebox.showinfo("Update Error","Empty GearDamage Codes Table. Please Select At least One Entries In The Table To Update GearDamage Code")
    ...
    def GetSetCatchQCFailedDB():
        try:
            ...
            if len(Complete_df) >0:
                ...
                SetCatchQCFailedDB_DF = SetCatchQCFailedDB_DF.loc[:,['DataBase_ID','RecordIdentifier','DeploymentUID','GearDamage','DeploymentIdentifier']]
    ...
    def RefFailedToSetcatchDB():
        ...
        SetCatchProfileDB_DF['GearDamage'] = (SetCatchProfileDB_DF.loc[:,['GearDamage']]).astype(int, errors='ignore')
        ...
        SetCatchQCFailedDB_DF['GearDamage'] = (SetCatchQCFailedDB_DF.loc[:,['GearDamage']]).replace('', 99999999)
        SetCatchQCFailedDB_DF['GearDamage'] = (SetCatchQCFailedDB_DF.loc[:,['GearDamage']]).astype(int, errors='ignore')

        Ref_FailedQC_InSetcatchDB = SetCatchProfileDB_DF.merge(
                                    SetCatchQCFailedDB_DF, on = ["DataBase_ID", "RecordIdentifier", "DeploymentUID", "GearDamage"], indicator=True, 
        ...
        Ref_FailedQC_InSetcatchDB['GearDamage'] = (Ref_FailedQC_InSetcatchDB.loc[:,['GearDamage']]).astype(int, errors='ignore')
        Ref_FailedQC_InSetcatchDB['GearDamage'] = (Ref_FailedQC_InSetcatchDB.loc[:,['GearDamage']]).replace(99999999, '')
        ...
        root = tk.Tk()
        root.title ("DFO-NL-ASOP GearDamage Codes Validation Failed On Set & Catch Imported Data")
    ...
    def SubmitDataFrameToQCFailedDB(DataFrameToSubmit):
        try:
            ...
            Import_To_DBStorage.to_sql('SetCatch_QCFailedLookUpTable_GearDamage', sqliteConnection, if_exists="replace", index =False)
    ...
    def GenDepIdfier_Col_SetCatchFailDB():
        QCFailedGearDamageDB= GetSetCatchQCFailedDB()
        if len(QCFailedGearDamageDB)>0:
            txtTotalFailedEntries.delete(0,END)
            QCFailedGearDamageDB['DeploymentIdentifier'] = QCFailedGearDamageDB['DeploymentUID'].apply(lambda x: '-'.join(y[:-4] if i == len(x.split('-'))-1 else y for i,y in enumerate(x.split('-'))))
            QCFailedGearDamageDB['DeploymentIdentifier'] = QCFailedGearDamageDB['DeploymentIdentifier'].apply(lambda x: x[:-1] if x.endswith('-') else x)
            QCFailedGearDamageDB = QCFailedGearDamageDB.reset_index(drop=True)
            DataFrameToSubmit = pd.DataFrame(QCFailedGearDamageDB)
            SubmitDataFrameToQCFailedDB(DataFrameToSubmit)
            QCFailedTotalEntries = len(DataFrameToSubmit)       
            txtTotalFailedEntries.insert(tk.END,QCFailedTotalEntries)

    def ClearGearDamageFailedSummaryTable_B():
        tree2.delete(*tree2.get_children())
        txtSelectedFailedEntries.delete(0,END)

    def ClearGearDamageFailedResultsTable_A():
        tree1.delete(*tree1.get_children())
        txtTotalFailedEntries.delete(0,END)
    ...
    def GenQCFailedSummaryTable():
        ClearGearDamageFailedSummaryTable_B()
        QCFailedGearDamageDB= GetSetCatchQCFailedDB()
        QCFailedGearDamageDB['GearDamage'] = (QCFailedGearDamageDB.loc[:,['GearDamage']]).replace(99999999, '')
        QCFailedGearDamageDB['GearDamage'] = (QCFailedGearDamageDB.loc[:,['GearDamage']]).astype(int, errors='ignore')
        QCFailedGearDamageDB   = QCFailedGearDamageDB.groupby(['DeploymentIdentifier', 'GearDamage'], as_index=False).DataBase_ID.count()
        QCFailedGearDamageDB   = pd.DataFrame(QCFailedGearDamageDB)
        QCFailedGearDamageDB.rename(columns={'DeploymentIdentifier':'DeploymentIdentifier',
        ...
        QCFailedGearDamageDB = pd.DataFrame(QCFailedGearDamageDB)
        countIndex1 = 0
        for each_rec in range(len(QCFailedGearDamageDB)):
            if countIndex1 % 2 == 0:
                tree2.insert("", tk.END, values=list(QCFailedGearDamageDB.loc[each_rec]), tags =("even",))
            else:
                tree2.insert("", tk.END, values=list(QCFailedGearDamageDB.loc[each_rec]), tags =("odd",))
    ...
    def Search_AND_MultiVariableQuery_Backend(DeploymentIdentifier ="", GearDamage =""):
        try:
            ...
            cur_DB_SetCatch_Validation_LookUpTable.execute("SELECT * FROM SetCatch_QCFailedLookUpTable_GearDamage WHERE \
                                            (DeploymentIdentifier = :DeploymentIdentifier OR TRIM(COALESCE(:DeploymentIdentifier, '')) = '') AND\
                                            (GearDamage= :GearDamage)",\
                                            (DeploymentIdentifier, GearDamage))
            rows=cur_DB_SetCatch_Validation_LookUpTable.fetchall()
            return rows
        except:
            messagebox.showerror('GearDamage Variable Error Message', "GearDamage Query Failed")
    ...
    def InventoryRec2(event):
        for nm in tree2.selection():
            ...
            GearDamage = sd[1]
            MultiSearchRows = Search_AND_MultiVariableQuery_Backend(DeploymentIdentifier, GearDamage)
            MultiSearchRowsDF = pd.DataFrame(MultiSearchRows, columns =['DataBase_ID', 'RecordIdentifier', 'DeploymentUID','GearDamage','DeploymentIdentifier'])
            MultiSearchRowsDF = MultiSearchRowsDF.loc[:,['DataBase_ID','RecordIdentifier','DeploymentUID','GearDamage','DeploymentIdentifier']]
    ...
    def SummarySelectUpdateClearFromQCFailDB():
        ...
        if len(ListBox_DF)>0:
            ...
            if Length_Selected != 0:
                ...
                if (len(SelectionTree)>0) & (len(SelectionTree)==1) & (len(SelectionTree)<2):
                    application_window = window
                    Input_UpdateAnswer = simpledialog.askstring("Input Updated GearDamage Code", "What is Your Updated GearDamage Code Value?", parent=application_window)
                    if Input_UpdateAnswer is not None:
                        Input_UpdatedGearDamageCode = (Input_UpdateAnswer)
                        GearDamageCodeFound = "Modied/Updated GearDamage Code Found In GearDamage lookup Table"
                        GearDamageCodeNotFound = "Modied/Updated GearDamage Code Not Found In GearDamage lookup Table"
                        ...
                        if EntryValidityGearDamageCode == GearDamageCodeFound:
                            ...
                            for child in tree1.get_children():
                                ...
                                cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET GearDamage = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (Input_UpdatedGearDamageCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                cur_DB_SetCatch_Validation_LookupTables.execute("DELETE FROM SetCatch_QCFailedLookUpTable_GearDamage WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID = ? ", 
            ...
            tkinter.messagebox.showinfo("Update Error","Empty GearDamage Summary Table : B")
    ...
    btnModifyGearDamageProfile = Button(TopLeftframe, text="Modify Code & Update DB", font=('aerial', 10, 'bold'), 
                            height =1, width=25, bd=2, command = Modify_SetCatch_GearDamageProfile)
    btnModifyGearDamageProfile.grid(row =10, column = 0, padx=2, pady =5, ipady =5, sticky =W)

    btnClearDetails = Button(TopLeftframe, text="Clear Details", font=('aerial', 10, 'bold'),
                            height =1, width=12, bd=1, command = ClearQCFailedGearDamageDetails)
    btnClearDetails.grid(row =10, column = 0, padx=2, pady =5, ipady =5, sticky =E)

    ## Button Wizard : Table B
    btnGenQCFailedGearDamageSummary = Button(TopBottomLeftframe, text="Generate Failed Summary", font=('aerial', 11, 'bold'), bg='alice blue',
                            height =1, width=21, bd=2, command = GenQCFailedSummaryTable)
    btnGenQCFailedGearDamageSummary.grid(row =2, column = 0, padx=2, pady =4, ipady =8, sticky =W)
    ...
    btnClearGearDamageTable = Button(TopRightframe, text="Clear All Table", font=('aerial', 11, 'bold'), 
                                height =1, width=12, bd=1, command = ClearAllGearDamageQCFailedTable)
    btnClearGearDamageTable.grid(row =2, column = 0, padx=2, pady =1, ipady =2, sticky =E)


    btnViewQCFailedQCGearDamageResults = Button(TopRightframe, text="View QC-Failed Results", font=('aerial', 11, 'bold'), bg='alice blue',
                            height =1, width=22, bd=2, command = viewQCFailed_GearDamageProfile)
    btnViewQCFailedQCGearDamageResults.grid(row =4, column = 0, padx=2, pady =2, ipady =4, sticky =W)
    ...
    filemenu.add_command(label="Export DB All QC Failed Results", command=Export_QCFailedGearDamage_CSV)

    Treepopup.add_command(label="Modify & Update DB With Selected GearDamage In Table C", command=Modify_MultipleSetCatch_GearDamageProfile)
