...
def SetCatch_ViewLookupValidatedResult_GearType():
    window = tk.Tk()
    window.title ("DFO-NL-ASOP Lookup GearType Codes Validator - ID-C-01-4")
    ...
    GearType   = IntVar(TopLeftframe, value='')

    lblTitEntry = Label(TopLeftframe, font=('aerial', 12, 'bold'), text=" A: GearType QC Failed Profile Details :")
    ...
    lblGearTypeCode = Label(TopLeftframe, font=('aerial', 10, 'bold'), text = "4. GearType :", padx =0, pady= 2)
    lblGearTypeCode.grid(row =8, column = 0, padx=4, pady =2, sticky =W)
    txtGearTypeCode  = Entry(TopLeftframe, font=('aerial', 12, 'bold'),textvariable= GearType, width = 35)
    txtGearTypeCode.grid(row =9, column = 0, padx=4, pady =4,ipadx=1, sticky =W)
    ...
    lblQCFailedResultGearType = Label(TopRightframe, font=('aerial', 11, 'bold'), text="C: View QC Failed Results Table")
    lblQCFailedResultGearType.grid(row =2, column = 0, padx=5, pady =2, ipady =2, sticky =W, rowspan =1)
    ...
    tree1.heading("#4", text="GearType", anchor=CENTER)
    ...
    tree2.heading("#2", text="GearType", anchor=CENTER)
    ...
    def GetGearTypeProfileDB():
        try:
            ...
            if len(Complete_df) >0:
                ...
                messagebox.showerror('GearType Code Lookup Table Message', 
                                    "Void GearType Code Lookup Table...Please Import/Insert GearType Coded Lookup Table")
    ...
    def viewQCFailed():
        try:
            ...
            cur=con.cursor()
            cur.execute("SELECT DataBase_ID, RecordIdentifier, DeploymentUID, GearType FROM SetCatch_QCFailedLookUpTable_GearType ORDER BY `DataBase_ID` ASC")
            rows=cur.fetchall()
            return rows
        except sqlite3.Error as error:
            print('Error occured - ', error)
    ...
    def viewQCFailed_GearTypeProfile():
        ClearGearTypeFailedResultsTable_A()
        countIndex = 0
        rows = viewQCFailed()
        for row in rows:
            if countIndex % 2 == 0:
    ...
    def QCFailedTotalEntries():
        ...
        Complete_df= pd.read_sql_query("SELECT * FROM SetCatch_QCFailedLookUpTable_GearType ORDER BY `DataBase_ID` ASC;", conn)
    ...
    def ClearQCFailedGearTypeDetails():
        txtDatabaseID.delete(0,END)
        txtRecordIdentifier.delete(0,END)
        txtDeploymentUID.delete(0,END)
        txtGearTypeCode.delete(0,END)
        
    def CheckGearTypeCodeValidity(UserEntryGearTypeCode):
        GearTypeCodeFound = "Modied/Updated GearType Code Found In GearType lookup Table"
        GearTypeCodeNotFound = "Modied/Updated GearType Code Not Found In GearType lookup Table"
        if((UserEntryGearTypeCode!='')):
            try:
                UserEntryGearTypeCode = int(UserEntryGearTypeCode)
                Complete_df = GetGearTypeProfileDB()
                data = pd.DataFrame(Complete_df)
                data = data.reset_index(drop=True)
                if (UserEntryGearTypeCode in data["GearType"].unique()) == True: 
                    return GearTypeCodeFound
                else: 
                    return GearTypeCodeNotFound
            except:
                return tkinter.messagebox.showerror("Update Error"," GearType Must Be Integer Value")
        else:
                return tkinter.messagebox.showerror("Update Error"," GearType Code Entry Can Not Be Empty")
        
    def Modify_SetCatch_GearTypeProfile():
        GearTypeCodeFound = "Modied/Updated GearType Code Found In GearType lookup Table"
        GearTypeCodeNotFound = "Modied/Updated GearType Code Not Found In GearType lookup Table"
        UserEntryGearTypeCode = txtGearTypeCode.get()
        EntryValidityGearTypeCode = CheckGearTypeCodeValidity(UserEntryGearTypeCode)
        if EntryValidityGearTypeCode == GearTypeCodeFound:
            cur_id = tree1.focus()
                    ...
                    if iUpdate >0:
                        ...
                        if (list_item_DatabaseUID == DataBase_ID) & \
                            ...
                            if(GearType!=0):
                                ...
                                cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET GearType = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (GearType, DataBase_ID, RecordIdentifier, DeploymentUID))
                                cur_DB_SetCatch_Validation_LookupTables.execute("UPDATE SetCatch_QCFailedLookUpTable_GearType SET GearType = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (GearType, DataBase_ID, RecordIdentifier, DeploymentUID))
                                ...
                                tree1.insert("", tk.END,values=(DataBase_ID, RecordIdentifier, DeploymentUID, GearType ))
                                QCFailedTotalEntries()
                                ClearGearTypeFailedSummaryTable_B()
                            else:
                                tkinter.messagebox.showerror("Update Error"," GearType Code Entry can not be empty")
                        else:
            ...
            tkinter.messagebox.showerror("Update/Modification Error In Set & Catch DB", GearTypeCodeNotFound + ".  " +  "Please Update GearType Code Lookup Table With The Correct GearType Code")

    def ClearAllGearTypeQCFailedTable():
        tree1.delete(*tree1.get_children())
    ...
    def InventoryRec1(event):
        for nm in tree1.selection():
            ...
            txtGearTypeCode.delete(0,END)
            txtGearTypeCode.insert(tk.END,sd[3])

    def Export_QCFailedGearType_CSV():
        try:
            conn = sqlite3.connect(DB_SetCatch_Validation_LookUpTable)
            cursor = conn.cursor()
            Complete_df = pd.read_sql_query("SELECT * FROM SetCatch_QCFailedLookUpTable_GearType ORDER BY `DataBase_ID` ASC ;", conn)
            if len(Complete_df) >0:
                ...
                if len(filename) >0:
                    ...
                    tkinter.messagebox.showinfo("QC Failed GearType Profile","QC Failed GearType Profile Report Saved as CSV")
                else:
                    tkinter.messagebox.showinfo("QC Failed GearType Profile Report Message","Please Select File Name To Export")
            else:
    ...
    def Modify_MultipleSetCatch_GearTypeProfile():
        dfList =[] 
            ...
            if Length_Selected != 0:
                ...
                if (len(SelectionTree)>0):
                    iUpdate = tkinter.messagebox.askyesno("Update Multiple Entries In  DFO-NL-ASOP Set & Catch Database", 
                            "Confirm If You Want To Update Multiple GearType Code Entries In DFO-NL-ASOP Set & Catch Database")
                    if iUpdate >0:
                        application_window = window
                        Input_UpdateAnswer = simpledialog.askstring("Input Updated GearType Code", "What is Your Updated GearType Code Value?",
                                                                parent=application_window)
                        if Input_UpdateAnswer is not None:
                            Input_UpdatedGearTypeCode = (Input_UpdateAnswer)
                            GearTypeCodeFound = "Modied/Updated GearType Code Found In GearType lookup Table"
                            GearTypeCodeNotFound = "Modied/Updated GearType Code Not Found In GearType lookup Table"
                            EntryValidityGearTypeCode = CheckGearTypeCodeValidity(Input_UpdatedGearTypeCode)
                            if EntryValidityGearTypeCode == GearTypeCodeFound:
                                conn_DB_Set_Catch_Analysis= sqlite3.connect(DB_Set_Catch_Analysis)
                                cur_DB_Set_Catch_Analysis=conn_DB_Set_Catch_Analysis.cursor()
                                conn_DB_SetCatch_Validation_LookupTables= sqlite3.connect(DB_SetCatch_Validation_LookUpTable)
                                ...
                                for item in SelectionTree:
                                    ...
                                    list_item_RecordIdentifier = int(list_item[1])
                                    list_item_DeploymentUID = (list_item[2])
                                    Input_UpdatedGearTypeCode =int(Input_UpdatedGearTypeCode)
                                    cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET GearType = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (Input_UpdatedGearTypeCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                    cur_DB_SetCatch_Validation_LookupTables.execute("UPDATE SetCatch_QCFailedLookUpTable_GearType SET GearType = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                        (Input_UpdatedGearTypeCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                conn_DB_Set_Catch_Analysis.commit()
                                conn_DB_Set_Catch_Analysis.close()
                                conn_DB_SetCatch_Validation_LookupTables.commit()
                                conn_DB_SetCatch_Validation_LookupTables.close()
                                viewQCFailed_GearTypeProfile()
                                ClearGearTypeFailedSummaryTable_B()
                                return
                            else:
                                tkinter.messagebox.showerror("Update/Modification Error In Set & Catch DB",
                                                            GearTypeCodeNotFound + ".  " +  
                                                            "Please Update GearType Code Lookup Table With The Correct GearType Code")
                        else:
                            tkinter.messagebox.showinfo("Update Error","Please Input Updated GearType Code Value")   
            else:
                tkinter.messagebox.showinfo("Update Error","Please Select At least One Entries To Update GearType Code")
        else:
            tkinter.messagebox.showinfo("Update Error","Empty GearType Codes Table. Please Select At least One Entries In The Table To Update GearType Code")
    ...
    def GetSetCatchQCFailedDB():
        try:
            ...
            if len(Complete_df) >0:
                ...
                SetCatchQCFailedDB_DF = SetCatchQCFailedDB_DF.loc[:,['DataBase_ID','RecordIdentifier','DeploymentUID','GearType','DeploymentIdentifier']]
    ...
    def RefFailedToSetcatchDB():
        ...
        SetCatchQCFailedDB_DF['GearType'] = (SetCatchQCFailedDB_DF.loc[:,['GearType']]).replace('', 99999999)
        SetCatchQCFailedDB_DF['GearType'] = (SetCatchQCFailedDB_DF.loc[:,['GearType']]).astype(int, errors='ignore')

        Ref_FailedQC_InSetcatchDB = SetCatchProfileDB_DF.merge(
                                    SetCatchQCFailedDB_DF, on = ["DataBase_ID", "RecordIdentifier", "DeploymentUID", "GearType"], indicator=True, 
        ...
        Ref_FailedQC_InSetcatchDB['GearType'] = (Ref_FailedQC_InSetcatchDB.loc[:,['GearType']]).astype(int, errors='ignore')
        Ref_FailedQC_InSetcatchDB['GearType'] = (Ref_FailedQC_InSetcatchDB.loc[:,['GearType']]).replace(99999999, '')
        ...
        root = tk.Tk()
        root.title ("DFO-NL-ASOP GearType Codes Validation Failed On Set & Catch Imported Data")
    ...
    def SubmitDataFrameToQCFailedDB(DataFrameToSubmit):
        try:
            ...
            Import_To_DBStorage.to_sql('SetCatch_QCFailedLookUpTable_GearType', sqliteConnection, if_exists="replace", index =False)
    ...
    def GenDepIdfier_Col_SetCatchFailDB():
        QCFailedGearTypeDB= GetSetCatchQCFailedDB()
        if len(QCFailedGearTypeDB)>0:
            txtTotalFailedEntries.delete(0,END)
            QCFailedGearTypeDB['DeploymentIdentifier'] = QCFailedGearTypeDB['DeploymentUID'].apply(lambda x: '-'.join(y[:-4] if i == len(x.split('-'))-1 else y for i,y in enumerate(x.split('-'))))
            QCFailedGearTypeDB['DeploymentIdentifier'] = QCFailedGearTypeDB['DeploymentIdentifier'].apply(lambda x: x[:-1] if x.endswith('-') else x)
            QCFailedGearTypeDB = QCFailedGearTypeDB.reset_index(drop=True)
            DataFrameToSubmit = pd.DataFrame(QCFailedGearTypeDB)
            SubmitDataFrameToQCFailedDB(DataFrameToSubmit)
            QCFailedTotalEntries = len(DataFrameToSubmit)       
            txtTotalFailedEntries.insert(tk.END,QCFailedTotalEntries)

    def ClearGearTypeFailedSummaryTable_B():
        tree2.delete(*tree2.get_children())
        txtSelectedFailedEntries.delete(0,END)

    def ClearGearTypeFailedResultsTable_A():
        tree1.delete(*tree1.get_children())
        txtTotalFailedEntries.delete(0,END)
    ...
    def GenQCFailedSummaryTable():
        ClearGearTypeFailedSummaryTable_B()
        QCFailedGearTypeDB= GetSetCatchQCFailedDB()
        QCFailedGearTypeDB['GearType'] = (QCFailedGearTypeDB.loc[:,['GearType']]).replace(99999999, '')
        QCFailedGearTypeDB['GearType'] = (QCFailedGearTypeDB.loc[:,['GearType']]).astype(int, errors='ignore')
        QCFailedGearTypeDB   = QCFailedGearTypeDB.groupby(['DeploymentIdentifier', 'GearType'], as_index=False).DataBase_ID.count()
        QCFailedGearTypeDB   = pd.DataFrame(QCFailedGearTypeDB)
        QCFailedGearTypeDB.rename(columns={'DeploymentIdentifier':'DeploymentIdentifier',
                                            'GearType': 'GearType', 
        ...
        QCFailedGearTypeDB = pd.DataFrame(QCFailedGearTypeDB)
        countIndex1 = 0
        for each_rec in range(len(QCFailedGearTypeDB)):
            if countIndex1 % 2 == 0:
                tree2.insert("", tk.END, values=list(QCFailedGearTypeDB.loc[each_rec]), tags =("even",))
            else:
                tree2.insert("", tk.END, values=list(QCFailedGearTypeDB.loc[each_rec]), tags =("odd",))
    ...
    def Search_AND_MultiVariableQuery_Backend(DeploymentIdentifier ="", GearType =""):
        try:
            ...
            cur_DB_SetCatch_Validation_LookUpTable.execute("SELECT * FROM SetCatch_QCFailedLookUpTable_GearType WHERE \
                                            (DeploymentIdentifier = :DeploymentIdentifier OR TRIM(COALESCE(:DeploymentIdentifier, '')) = '') AND\
                                            (GearType= :GearType)",\
                                            (DeploymentIdentifier, GearType))
            rows=cur_DB_SetCatch_Validation_LookUpTable.fetchall()
            return rows
        except:
            messagebox.showerror('GearType Variable Error Message', "GearType Query Failed")
    ...
    def InventoryRec2(event):
        for nm in tree2.selection():
            ...
            GearType = sd[1]
            MultiSearchRows = Search_AND_MultiVariableQuery_Backend(DeploymentIdentifier, GearType)
            MultiSearchRowsDF = pd.DataFrame(MultiSearchRows, columns =['DataBase_ID', 'RecordIdentifier', 'DeploymentUID','GearType','DeploymentIdentifier'])
            MultiSearchRowsDF = MultiSearchRowsDF.loc[:,['DataBase_ID','RecordIdentifier','DeploymentUID','GearType','DeploymentIdentifier']]
    ...
    def SummarySelectUpdateClearFromQCFailDB():
        ...
        if len(ListBox_DF)>0:
            ...
            if Length_Selected != 0:
                ...
                if (len(SelectionTree)>0) & (len(SelectionTree)==1) & (len(SelectionTree)<2):
                    application_window = window
                    Input_UpdateAnswer = simpledialog.askstring("Input Updated GearType Code", "What is Your Updated GearType Code Value?", parent=application_window)
                    if Input_UpdateAnswer is not None:
                        Input_UpdatedGearTypeCode = (Input_UpdateAnswer)
                        GearTypeCodeFound = "Modied/Updated GearType Code Found In GearType lookup Table"
                        GearTypeCodeNotFound = "Modied/Updated GearType Code Not Found In GearType lookup Table"
                        ...
                        if EntryValidityGearTypeCode == GearTypeCodeFound:
                            ...
                            for child in tree1.get_children():
                                ...
                                cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET GearType = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (Input_UpdatedGearTypeCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                cur_DB_SetCatch_Validation_LookupTables.execute("DELETE FROM SetCatch_QCFailedLookUpTable_GearType WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID = ? ", 
            ...
            tkinter.messagebox.showinfo("Update Error","Empty GearType Summary Table : B")
    ...
    btnModifyGearTypeProfile = Button(TopLeftframe, text="Modify Code & Update DB", font=('aerial', 10, 'bold'), 
                            height =1, width=25, bd=2, command = Modify_SetCatch_GearTypeProfile)
    btnModifyGearTypeProfile.grid(row =10, column = 0, padx=2, pady =5, ipady =5, sticky =W)

    btnClearDetails = Button(TopLeftframe, text="Clear Details", font=('aerial', 10, 'bold'),
                            height =1, width=12, bd=1, command = ClearQCFailedGearTypeDetails)
    btnClearDetails.grid(row =10, column = 0, padx=2, pady =5, ipady =5, sticky =E)

    ## Button Wizard : Table B
    btnGenQCFailedGearTypeSummary = Button(TopBottomLeftframe, text="Generate Failed Summary", font=('aerial', 11, 'bold'), bg='alice blue',
                            height =1, width=21, bd=2, command = GenQCFailedSummaryTable)
    btnGenQCFailedGearTypeSummary.grid(row =2, column = 0, padx=2, pady =4, ipady =8, sticky =W)
    ...
    btnClearGearTypeTable = Button(TopRightframe, text="Clear All Table", font=('aerial', 11, 'bold'), 
                                height =1, width=12, bd=1, command = ClearAllGearTypeQCFailedTable)
    btnClearGearTypeTable.grid(row =2, column = 0, padx=2, pady =1, ipady =2, sticky =E)


    btnViewQCFailedQCGearTypeResults = Button(TopRightframe, text="View QC-Failed Results", font=('aerial', 11, 'bold'), bg='alice blue',
                            height =1, width=22, bd=2, command = viewQCFailed_GearTypeProfile)
    btnViewQCFailedQCGearTypeResults.grid(row =4, column = 0, padx=2, pady =2, ipady =4, sticky =W)
    ...
    filemenu.add_command(label="Export DB All QC Failed Results", command=Export_QCFailedGearType_CSV)

    Treepopup.add_command(label="Modify & Update DB With Selected GearType In Table C", command=Modify_MultipleSetCatch_GearTypeProfile)
