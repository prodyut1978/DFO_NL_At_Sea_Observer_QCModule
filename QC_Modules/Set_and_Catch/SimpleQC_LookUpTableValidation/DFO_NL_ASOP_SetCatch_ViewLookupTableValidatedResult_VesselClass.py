...
def SetCatch_ViewLookupValidatedResult_VesselClass():
    window = tk.Tk()
    window.title ("DFO-NL-ASOP Lookup VesselClass Codes Validator - ID-C-01-11")
    ...
    VesselClass   = IntVar(TopLeftframe, value='')

    lblTitEntry = Label(TopLeftframe, font=('aerial', 12, 'bold'), text=" A: VesselClass QC Failed Profile Details :")
    ...
    lblVesselClassCode = Label(TopLeftframe, font=('aerial', 10, 'bold'), text = "4. VesselClass :", padx =0, pady= 2)
    lblVesselClassCode.grid(row =8, column = 0, padx=4, pady =2, sticky =W)
    txtVesselClassCode  = Entry(TopLeftframe, font=('aerial', 12, 'bold'),textvariable= VesselClass, width = 35)
    txtVesselClassCode.grid(row =9, column = 0, padx=4, pady =4,ipadx=1, sticky =W)
    ...
    lblQCFailedResultVesselClass = Label(TopRightframe, font=('aerial', 11, 'bold'), text="C: View QC Failed Results Table")
    lblQCFailedResultVesselClass.grid(row =2, column = 0, padx=5, pady =2, ipady =2, sticky =W, rowspan =1)
    ...
    tree1.heading("#4", text="VesselClass", anchor=CENTER)
    ...
    tree2.heading("#2", text="VesselClass", anchor=CENTER)
    ...
    def GetVesselClassProfileDB():
        try:
            ...
            if len(Complete_df) >0:
                ...
                messagebox.showerror('VesselClass Code Lookup Table Message', 
                                    "Void VesselClass Code Lookup Table...Please Import/Insert VesselClass Coded Lookup Table")
    ...
    def viewQCFailed():
        try:
            ...
            cur=con.cursor()
            cur.execute("SELECT DataBase_ID, RecordIdentifier, DeploymentUID, VesselClass FROM SetCatch_QCFailedLookUpTable_VesselClass ORDER BY `DataBase_ID` ASC")
            rows=cur.fetchall()
            return rows
        except sqlite3.Error as error:
            print('Error occured - ', error)
    ...
    def viewQCFailed_VesselClassProfile():
        ClearVesselClassFailedResultsTable_A()
        countIndex = 0
        rows = viewQCFailed()
        for row in rows:
            if countIndex % 2 == 0:
    ...
    def QCFailedTotalEntries():
        ...
        Complete_df= pd.read_sql_query("SELECT * FROM SetCatch_QCFailedLookUpTable_VesselClass ORDER BY `DataBase_ID` ASC;", conn)
    ...
    def ClearQCFailedVesselClassDetails():
        txtDatabaseID.delete(0,END)
        txtRecordIdentifier.delete(0,END)
        txtDeploymentUID.delete(0,END)
        txtVesselClassCode.delete(0,END)
        
    def CheckVesselClassCodeValidity(UserEntryVesselClassCode):
        VesselClassCodeFound = "Modied/Updated VesselClass Code Found In VesselClass lookup Table"
        VesselClassCodeNotFound = "Modied/Updated VesselClass Code Not Found In VesselClass lookup Table"
        if((UserEntryVesselClassCode!='')):
            try:
                UserEntryVesselClassCode = int(UserEntryVesselClassCode)
                Complete_df = GetVesselClassProfileDB()
                data = pd.DataFrame(Complete_df)
                data = data.reset_index(drop=True)
                if (UserEntryVesselClassCode in data["VesselClass"].unique()) == True: 
                    return VesselClassCodeFound
                else: 
                    return VesselClassCodeNotFound
            except:
                return tkinter.messagebox.showerror("Update Error"," VesselClass Must Be Integer Value")
        else:
                return tkinter.messagebox.showerror("Update Error"," VesselClass Code Entry Can Not Be Empty")
        
    def Modify_SetCatch_VesselClassProfile():
        VesselClassCodeFound = "Modied/Updated VesselClass Code Found In VesselClass lookup Table"
        VesselClassCodeNotFound = "Modied/Updated VesselClass Code Not Found In VesselClass lookup Table"
        UserEntryVesselClassCode = txtVesselClassCode.get()
        EntryValidityVesselClassCode = CheckVesselClassCodeValidity(UserEntryVesselClassCode)
        if EntryValidityVesselClassCode == VesselClassCodeFound:
            cur_id = tree1.focus()
            ...
            if Length_Selected != 0:
                ...
                if (len(SelectionTree)>0) & (len(SelectionTree)==1) & (len(SelectionTree)<2):
                    ...
                    if iUpdate >0:
                        ...
                        VesselClass = txtVesselClassCode.get()
                        if (list_item_DatabaseUID == DataBase_ID) & \
                            ...
                            if(VesselClass!=0):
                                ...
                                cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET VesselClass = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (VesselClass, DataBase_ID, RecordIdentifier, DeploymentUID))
                                cur_DB_SetCatch_Validation_LookupTables.execute("UPDATE SetCatch_QCFailedLookUpTable_VesselClass SET VesselClass = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (VesselClass, DataBase_ID, RecordIdentifier, DeploymentUID))
                                ...
                                tree1.insert("", tk.END,values=(DataBase_ID, RecordIdentifier, DeploymentUID, VesselClass ))
                                QCFailedTotalEntries()
                                ClearVesselClassFailedSummaryTable_B()
                            else:
                                tkinter.messagebox.showerror("Update Error"," VesselClass Code Entry can not be empty")
                        else:
            ...
            tkinter.messagebox.showerror("Update/Modification Error In Set & Catch DB", VesselClassCodeNotFound + ".  " +  "Please Update VesselClass Code Lookup Table With The Correct VesselClass Code")

    def ClearAllVesselClassQCFailedTable():
        tree1.delete(*tree1.get_children())
    ...
    def InventoryRec1(event):
        for nm in tree1.selection():
            ...
            txtVesselClassCode.delete(0,END)
            txtVesselClassCode.insert(tk.END,sd[3])

    def Export_QCFailedVesselClass_CSV():
        try:
            conn = sqlite3.connect(DB_SetCatch_Validation_LookUpTable)
            cursor = conn.cursor()
            Complete_df = pd.read_sql_query("SELECT * FROM SetCatch_QCFailedLookUpTable_VesselClass ORDER BY `DataBase_ID` ASC ;", conn)
            if len(Complete_df) >0:
                ...
                if len(filename) >0:
                    ...
                    tkinter.messagebox.showinfo("QC Failed VesselClass Profile","QC Failed VesselClass Profile Report Saved as CSV")
                else:
                    tkinter.messagebox.showinfo("QC Failed VesselClass Profile Report Message","Please Select File Name To Export")
            else:
    ...
    def Modify_MultipleSetCatch_VesselClassProfile():
        dfList =[] 
            ...
            if Length_Selected != 0:
                ...
                if (len(SelectionTree)>0):
                    iUpdate = tkinter.messagebox.askyesno("Update Multiple Entries In  DFO-NL-ASOP Set & Catch Database", 
                            "Confirm If You Want To Update Multiple VesselClass Code Entries In DFO-NL-ASOP Set & Catch Database")
                    if iUpdate >0:
                        application_window = window
                        Input_UpdateAnswer = simpledialog.askstring("Input Updated VesselClass Code", "What is Your Updated VesselClass Code Value?",
                                                                parent=application_window)
                        if Input_UpdateAnswer is not None:
                            Input_UpdatedVesselClassCode = (Input_UpdateAnswer)
                            VesselClassCodeFound = "Modied/Updated VesselClass Code Found In VesselClass lookup Table"
                            VesselClassCodeNotFound = "Modied/Updated VesselClass Code Not Found In VesselClass lookup Table"
                            EntryValidityVesselClassCode = CheckVesselClassCodeValidity(Input_UpdatedVesselClassCode)
                            if EntryValidityVesselClassCode == VesselClassCodeFound:
                                conn_DB_Set_Catch_Analysis= sqlite3.connect(DB_Set_Catch_Analysis)
                                cur_DB_Set_Catch_Analysis=conn_DB_Set_Catch_Analysis.cursor()
                                conn_DB_SetCatch_Validation_LookupTables= sqlite3.connect(DB_SetCatch_Validation_LookUpTable)
                                ...
                                for item in SelectionTree:
                                    ...
                                    list_item_RecordIdentifier = int(list_item[1])
                                    list_item_DeploymentUID = (list_item[2])
                                    Input_UpdatedVesselClassCode =int(Input_UpdatedVesselClassCode)
                                    cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET VesselClass = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (Input_UpdatedVesselClassCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                    cur_DB_SetCatch_Validation_LookupTables.execute("UPDATE SetCatch_QCFailedLookUpTable_VesselClass SET VesselClass = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                        (Input_UpdatedVesselClassCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                conn_DB_Set_Catch_Analysis.commit()
                                conn_DB_Set_Catch_Analysis.close()
                                conn_DB_SetCatch_Validation_LookupTables.commit()
                                conn_DB_SetCatch_Validation_LookupTables.close()
                                viewQCFailed_VesselClassProfile()
                                ClearVesselClassFailedSummaryTable_B()
                                return
                            else:
                                tkinter.messagebox.showerror("Update/Modification Error In Set & Catch DB",
                                                            VesselClassCodeNotFound + ".  " +  
                                                            "Please Update VesselClass Code Lookup Table With The Correct VesselClass Code")
                        else:
                            tkinter.messagebox.showinfo("Update Error","Please Input Updated VesselClass Code Value")   
            else:
                tkinter.messagebox.showinfo("Update Error","Please Select At least One Entries To Update VesselClass Code")
        else:
            tkinter.messagebox.showinfo("Update Error","Empty VesselClass Codes Table. Please Select At least One Entries In The Table To Update VesselClass Code")
    ...
    def GetSetCatchQCFailedDB():
        try:
            ...
            if len(Complete_df) >0:
                ...
                SetCatchQCFailedDB_DF = SetCatchQCFailedDB_DF.loc[:,['DataBase_ID','RecordIdentifier','DeploymentUID','VesselClass','DeploymentIdentifier']]
    ...
    def RefFailedToSetcatchDB():
        ...
        SetCatchQCFailedDB_DF['VesselClass'] = (SetCatchQCFailedDB_DF.loc[:,['VesselClass']]).replace('', 99999999)
        SetCatchQCFailedDB_DF['VesselClass'] = (SetCatchQCFailedDB_DF.loc[:,['VesselClass']]).astype(int, errors='ignore')

        Ref_FailedQC_InSetcatchDB = SetCatchProfileDB_DF.merge(
                                    SetCatchQCFailedDB_DF, on = ["DataBase_ID", "RecordIdentifier", "DeploymentUID", "VesselClass"], indicator=True, 
        ...
        Ref_FailedQC_InSetcatchDB['VesselClass'] = (Ref_FailedQC_InSetcatchDB.loc[:,['VesselClass']]).astype(int, errors='ignore')
        Ref_FailedQC_InSetcatchDB['VesselClass'] = (Ref_FailedQC_InSetcatchDB.loc[:,['VesselClass']]).replace(99999999, '')
        ...
        root = tk.Tk()
        root.title ("DFO-NL-ASOP VesselClass Codes Validation Failed On Set & Catch Imported Data")
    ...
    def SubmitDataFrameToQCFailedDB(DataFrameToSubmit):
        try:
            ...
            Import_To_DBStorage.to_sql('SetCatch_QCFailedLookUpTable_VesselClass', sqliteConnection, if_exists="replace", index =False)
    ...
    def GenDepIdfier_Col_SetCatchFailDB():
        QCFailedVesselClassDB= GetSetCatchQCFailedDB()
        if len(QCFailedVesselClassDB)>0:
            txtTotalFailedEntries.delete(0,END)
            QCFailedVesselClassDB['DeploymentIdentifier'] = QCFailedVesselClassDB['DeploymentUID'].apply(lambda x: '-'.join(y[:-4] if i == len(x.split('-'))-1 else y for i,y in enumerate(x.split('-'))))
            QCFailedVesselClassDB['DeploymentIdentifier'] = QCFailedVesselClassDB['DeploymentIdentifier'].apply(lambda x: x[:-1] if x.endswith('-') else x)
            QCFailedVesselClassDB = QCFailedVesselClassDB.reset_index(drop=True)
            DataFrameToSubmit = pd.DataFrame(QCFailedVesselClassDB)
            SubmitDataFrameToQCFailedDB(DataFrameToSubmit)
            QCFailedTotalEntries = len(DataFrameToSubmit)       
            txtTotalFailedEntries.insert(tk.END,QCFailedTotalEntries)

    def ClearVesselClassFailedSummaryTable_B():
        tree2.delete(*tree2.get_children())
        txtSelectedFailedEntries.delete(0,END)

    def ClearVesselClassFailedResultsTable_A():
        tree1.delete(*tree1.get_children())
        txtTotalFailedEntries.delete(0,END)
    ...
    def GenQCFailedSummaryTable():
        ClearVesselClassFailedSummaryTable_B()
        QCFailedVesselClassDB= GetSetCatchQCFailedDB()
        QCFailedVesselClassDB['VesselClass'] = (QCFailedVesselClassDB.loc[:,['VesselClass']]).replace(99999999, '')
        QCFailedVesselClassDB['VesselClass'] = (QCFailedVesselClassDB.loc[:,['VesselClass']]).astype(int, errors='ignore')
        QCFailedVesselClassDB   = QCFailedVesselClassDB.groupby(['DeploymentIdentifier', 'VesselClass'], as_index=False).DataBase_ID.count()
        QCFailedVesselClassDB   = pd.DataFrame(QCFailedVesselClassDB)
        QCFailedVesselClassDB.rename(columns={'DeploymentIdentifier':'DeploymentIdentifier',
        ...
        QCFailedVesselClassDB = pd.DataFrame(QCFailedVesselClassDB)
        countIndex1 = 0
        for each_rec in range(len(QCFailedVesselClassDB)):
            if countIndex1 % 2 == 0:
                tree2.insert("", tk.END, values=list(QCFailedVesselClassDB.loc[each_rec]), tags =("even",))
            else:
                tree2.insert("", tk.END, values=list(QCFailedVesselClassDB.loc[each_rec]), tags =("odd",))
    ...
    def Search_AND_MultiVariableQuery_Backend(DeploymentIdentifier ="", VesselClass =""):
        try:
            ...
            cur_DB_SetCatch_Validation_LookUpTable.execute("SELECT * FROM SetCatch_QCFailedLookUpTable_VesselClass WHERE \
                                            (DeploymentIdentifier = :DeploymentIdentifier OR TRIM(COALESCE(:DeploymentIdentifier, '')) = '') AND\
                                            (VesselClass= :VesselClass)",\
                                            (DeploymentIdentifier, VesselClass))
            rows=cur_DB_SetCatch_Validation_LookUpTable.fetchall()
            return rows
        except:
            messagebox.showerror('VesselClass Variable Error Message', "VesselClass Query Failed")
    ...
    def InventoryRec2(event):
        for nm in tree2.selection():
            ...
            VesselClass = sd[1]
            MultiSearchRows = Search_AND_MultiVariableQuery_Backend(DeploymentIdentifier, VesselClass)
            MultiSearchRowsDF = pd.DataFrame(MultiSearchRows, columns =['DataBase_ID', 'RecordIdentifier', 'DeploymentUID','VesselClass','DeploymentIdentifier'])
            MultiSearchRowsDF = MultiSearchRowsDF.loc[:,['DataBase_ID','RecordIdentifier','DeploymentUID','VesselClass','DeploymentIdentifier']]
    ...
    def SummarySelectUpdateClearFromQCFailDB():
        ...
        if len(ListBox_DF)>0:
            ...
            if Length_Selected != 0:
                ...
                if (len(SelectionTree)>0) & (len(SelectionTree)==1) & (len(SelectionTree)<2):
                    application_window = window
                    Input_UpdateAnswer = simpledialog.askstring("Input Updated VesselClass Code", "What is Your Updated VesselClass Code Value?", parent=application_window)
                    if Input_UpdateAnswer is not None:
                        Input_UpdatedVesselClassCode = (Input_UpdateAnswer)
                        VesselClassCodeFound = "Modied/Updated VesselClass Code Found In VesselClass lookup Table"
                        VesselClassCodeNotFound = "Modied/Updated VesselClass Code Not Found In VesselClass lookup Table"
                        ...
                        if EntryValidityVesselClassCode == VesselClassCodeFound:
                            ...
                            for child in tree1.get_children():
                                ...
                                cur_DB_Set_Catch_Analysis.execute("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET VesselClass = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                    (Input_UpdatedVesselClassCode, list_item_DatabaseUID, list_item_RecordIdentifier, list_item_DeploymentUID))
                                cur_DB_SetCatch_Validation_LookupTables.execute("DELETE FROM SetCatch_QCFailedLookUpTable_VesselClass WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID = ? ", 
            ...
            tkinter.messagebox.showinfo("Update Error","Empty VesselClass Summary Table : B")
    ...
    btnModifyVesselClassProfile = Button(TopLeftframe, text="Modify Code & Update DB", font=('aerial', 10, 'bold'), 
                            height =1, width=25, bd=2, command = Modify_SetCatch_VesselClassProfile)
    btnModifyVesselClassProfile.grid(row =10, column = 0, padx=2, pady =5, ipady =5, sticky =W)

    btnClearDetails = Button(TopLeftframe, text="Clear Details", font=('aerial', 10, 'bold'),
                            height =1, width=12, bd=1, command = ClearQCFailedVesselClassDetails)
    btnClearDetails.grid(row =10, column = 0, padx=2, pady =5, ipady =5, sticky =E)

    ## Button Wizard : Table B
    btnGenQCFailedVesselClassSummary = Button(TopBottomLeftframe, text="Generate Failed Summary", font=('aerial', 11, 'bold'), bg='alice blue',
                            height =1, width=21, bd=2, command = GenQCFailedSummaryTable)
    btnGenQCFailedVesselClassSummary.grid(row =2, column = 0, padx=2, pady =4, ipady =8, sticky =W)
    ...
    btnClearVesselClassTable = Button(TopRightframe, text="Clear All Table", font=('aerial', 11, 'bold'), 
                                height =1, width=12, bd=1, command = ClearAllVesselClassQCFailedTable)
    btnClearVesselClassTable.grid(row =2, column = 0, padx=2, pady =1, ipady =2, sticky =E)


    btnViewQCFailedQCVesselClassResults = Button(TopRightframe, text="View QC-Failed Results", font=('aerial', 11, 'bold'), bg='alice blue',
                            height =1, width=22, bd=2, command = viewQCFailed_VesselClassProfile)
    btnViewQCFailedQCVesselClassResults.grid(row =4, column = 0, padx=2, pady =2, ipady =4, sticky =W)
    ...
    filemenu.add_command(label="Export DB All QC Failed Results", command=Export_QCFailedVesselClass_CSV)

    Treepopup.add_command(label="Modify & Update DB With Selected VesselClass In Table C", command=Modify_MultipleSetCatch_VesselClassProfile)
