...
def ViewConsis_Calender_ValResult():
    window = tk.Tk()
    window.title ("DFO-NL-ASOP Consistency Validator - ID-C-05-3")
    window.geometry("1460x825+200+100")
    ...
    lbl_TopFrame = Label(TopFrame, font=('aerial', 10, 'bold'), bg= "cadet blue", text="A. QC Fail Table (Calender Variable) :")
    lbl_TopFrame.grid(row =1, column = 0, padx=2, pady =1, ipady=1, sticky =W)

    ListVariableListA = ['Select Calender (Y-M-D/Y-HM-HD) Consistency View Type', 
                         'View Calender Consistency Fail With RecType-1,2',
                         'View Calender Consistency Fail With RecType-1 Only']
    ...
    tree1.heading("#8", text="RecordType", anchor=CENTER)
    tree1.heading("#9", text="Year", anchor=CENTER)
    tree1.heading("#10", text="Day", anchor=CENTER)
    tree1.heading("#11", text="Month", anchor=CENTER)
    tree1.heading("#12", text="HaulDay", anchor=CENTER)
    tree1.heading("#13", text="HaulMonth", anchor=CENTER)
    tree1.heading("#14", text="StartTime", anchor=CENTER)
    tree1.heading("#15", text="Duration", anchor=CENTER)
    tree1.heading("#16", text="QC_Message", anchor=CENTER)
    ...
    tree1.column('#7', stretch=NO, minwidth=0, width=100, anchor = tk.CENTER)
    tree1.column('#8', stretch=NO, minwidth=0, width=100, anchor = tk.CENTER)            
    tree1.column('#9', stretch=NO, minwidth=0, width=70, anchor = tk.CENTER)
    tree1.column('#10', stretch=NO, minwidth=0, width=60, anchor = tk.CENTER)
    ...
    tree1.column('#13', stretch=NO, minwidth=0, width=80, anchor = tk.CENTER)
    tree1.column('#14', stretch=NO, minwidth=0, width=80, anchor = tk.CENTER)            
    tree1.column('#15', stretch=NO, minwidth=0, width=80, anchor = tk.CENTER)
    tree1.column('#16', stretch=NO, minwidth=0, width=200, anchor = tk.CENTER)
    ...
    ListUpdateVar = ['Day', 'Month', 'HaulDay', 'HaulMonth']
    ...
    lbl_SummaryQCframe = Label(SummaryQCframe, font=('aerial', 10, 'bold','underline'), 
                                bg= "cadet blue", text="B: QC Fail Summary (Calender Variables)")
    ...
    Summaryframetree.heading("#2", text="ObserverNumber")
    ...
    Summaryframetree.column('#2', stretch=NO, minwidth=0, width=120, anchor = tk.CENTER)
    Summaryframetree.column('#3', stretch=NO, minwidth=0, width=65, anchor = tk.CENTER)            
    Summaryframetree.column('#4', stretch=NO, minwidth=0, width=150, anchor = tk.CENTER)
    Summaryframetree.column('#5', stretch=NO, minwidth=0, width=150, anchor = tk.CENTER)
    ...
    QcValue = "For Current Set Year/Month/Day (Y-M-D) is Same Or Before Year/HaulMonth/HaulDay (Y-HM-HD)"
    txtSummaryDisplayMsg = Entry(SummaryDisplay, font=('aerial', 10),
                            textvariable = StringVar(window, value=QcValue), width = 84, bd=2)
    ...
    entry_SelQCVariable = Entry(SelQCVariableDisplay, font=('aerial', 10, 'bold'),  justify = tk.CENTER,
                            textvariable = SelQCVariable, width = 14, bd=2)
    ...
    entry_SearchNeighbour = Entry(SelQCVariableDisplay, font=('aerial', 10, 'bold'),  justify = tk.CENTER,
                            textvariable = SearchNeighbour, width = 28, bd=2)
    ...
    SelResultOverviewtree = ttk.Treeview(SelResultOverview,  
                            column=("column1", "column2", 
                                    "column3", "column4", 
                                    "column5", "column6", "column7", ), height=9, show='headings')
    ...
    SelResultOverviewtree.heading("#2", text="RecordType-1", anchor=CENTER)
    SelResultOverviewtree.heading("#3", text="Year", anchor=CENTER)
    SelResultOverviewtree.heading("#4", text="Day", anchor=CENTER)
    SelResultOverviewtree.heading("#5", text="Month", anchor=CENTER)
    SelResultOverviewtree.heading("#6", text="HaulDay", anchor=CENTER)
    SelResultOverviewtree.heading("#7", text="HaulMonth", anchor=CENTER)
    SelResultOverviewtree.column('#1', stretch=NO, minwidth=0, width=122, anchor = tk.CENTER)
    SelResultOverviewtree.column('#2', stretch=NO, minwidth=0, width=88, anchor = tk.CENTER)             
    SelResultOverviewtree.column('#3', stretch=NO, minwidth=0, width=75, anchor = tk.CENTER)
    SelResultOverviewtree.column('#4', stretch=NO, minwidth=0, width=40, anchor = tk.CENTER)
    SelResultOverviewtree.column('#5', stretch=NO, minwidth=0, width=57, anchor = tk.CENTER)
    SelResultOverviewtree.column('#6', stretch=NO, minwidth=0, width=70, anchor = tk.CENTER)
    SelResultOverviewtree.column('#7', stretch=NO, minwidth=0, width=65, anchor = tk.CENTER)
    style = ttk.Style(window)
    style.theme_use('clam')
                ...
                if filename.endswith('.csv'):
                    ...
                    if Return_Message == ReturnMatchedMessage:
                        ...
                        catdf.rename(columns=  {0:'DataBase_ID', 1: 'RecordIdentifier', 2: 'DeploymentUID',
                                                ...
                                                6:'SetNumber', 7:'RecordType', 
                                                8:'Year', 9:'Day', 10:'Month', 
                                                11:'HaulDay', 12:'HaulMonth',
                                                13:'StartTime', 14:'Duration', 
					                            15:'QC_Message'},inplace = True)
                    ...
                    if Len_CheckEmptyNessColumn==0:
                        ...
                        if Length_Raw_Imported_Df <250000:
                            ...
                            for row in df_rows:
                                ...
                                list_item_RecordType = (rowValue[7])
                                list_item_Year = (rowValue[8])
                                list_item_Day = (rowValue[9])
                                list_item_Month = (rowValue[10])
                                list_item_HaulDay = (rowValue[11])
                                list_item_HaulMonth = (rowValue[12])
                                list_item_StartTime = (rowValue[13])
                                list_item_Duration = (rowValue[14])
                                list_item_QCMsg = ''
                            ...
                            cur_DB_SetCatch_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF  SET ASOCCode = ?, \
                                    ObserverNumber = ?, DeploymentNumber = ?, SetNumber = ?,\
                                    RecordType = ?, Year = ?, Day = ?, Month =?,\
                                    HaulDay = ?, HaulMonth =? , StartTime = ?, Duration =?, QC_Message =?\
                                    WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
    ...
    def viewQCFailed(getVarnameToView):
        ...
        try:
            ...
            cur=con.cursor()
            cur.execute("SELECT * FROM SetCatch_DM_HDHM_FailConsistency_DF")
            rows=cur.fetchall()
    ...
    def viewQCFailed_VariablesProfile():
        ...
        if len(rows) >0 :
            ...
            if getVarnameToView == ListVariableListA[0]:
                txtDisplayMessageSystem.delete(0,END)
                txtDisplayMessageSystem.insert(0, 'Select View Type From DropDown & Run View Selected Button')
            else:
                countIndex1 = 0
                ...
                txtDisplayMessageSystem.delete(0,END)
                entry_SelQCVariable.delete(0,END)
                txtDisplayMessageSystem.insert(0, 'Populated Calender Consistency Fail Entries')
        else:
            txtDisplayMessageSystem.delete(0,END)
    ...
    def QCFailedExcelViewAll():
        conn = sqlite3.connect(DB_SetCatch_Validation_Consistency)
        cursor = conn.cursor()
        Calender_FailConsistency_DF = pd.read_sql_query("SELECT * FROM SetCatch_DM_HDHM_FailConsistency_DF;", conn)
        Calender_FailConsistency_DF = pd.DataFrame(Calender_FailConsistency_DF)
        cursor.close()
        conn.close()

        Calender_FailConsistency_DF = pd.DataFrame(Calender_FailConsistency_DF)
        Calender_FailConsistency_DF['DataBase_ID'] = (Calender_FailConsistency_DF.loc[:,['DataBase_ID']]).astype(int)
        Calender_FailConsistency_DF['RecordIdentifier'] = (Calender_FailConsistency_DF.loc[:,['RecordIdentifier']]).astype(int)
        Calender_FailConsistency_DF['DeploymentUID'] = (Calender_FailConsistency_DF.loc[:,['DeploymentUID']]).astype(str)

        Calender_FailConsistency_DF['ASOCCode'] = (Calender_FailConsistency_DF.loc[:,['ASOCCode']]).fillna(99999999)
        Calender_FailConsistency_DF['ASOCCode'] = (Calender_FailConsistency_DF.loc[:,['ASOCCode']]).replace([''], 99999999)
        Calender_FailConsistency_DF['ASOCCode'] = (Calender_FailConsistency_DF.loc[:,['ASOCCode']]).astype(int)

        Calender_FailConsistency_DF['ObserverNumber'] = (Calender_FailConsistency_DF.loc[:,['ObserverNumber']]).astype(str)
        
        Calender_FailConsistency_DF['DeploymentNumber'] = (Calender_FailConsistency_DF.loc[:,['DeploymentNumber']]).fillna(99999999)
        Calender_FailConsistency_DF['DeploymentNumber'] = (Calender_FailConsistency_DF.loc[:,['DeploymentNumber']]).replace([''], 99999999)
        Calender_FailConsistency_DF['DeploymentNumber'] = (Calender_FailConsistency_DF.loc[:,['DeploymentNumber']]).astype(int)
        
        Calender_FailConsistency_DF['SetNumber'] = (Calender_FailConsistency_DF.loc[:,['SetNumber']]).fillna(99999999)
        Calender_FailConsistency_DF['SetNumber'] = (Calender_FailConsistency_DF.loc[:,['SetNumber']]).replace([''], 99999999)
        Calender_FailConsistency_DF['SetNumber'] = (Calender_FailConsistency_DF.loc[:,['SetNumber']]).astype(int)
        
        Calender_FailConsistency_DF['RecordType'] = (Calender_FailConsistency_DF.loc[:,['RecordType']]).fillna(99999999)
        Calender_FailConsistency_DF['RecordType'] = (Calender_FailConsistency_DF.loc[:,['RecordType']]).replace([''], 99999999)
        Calender_FailConsistency_DF['RecordType'] = (Calender_FailConsistency_DF.loc[:,['RecordType']]).astype(int)
        
        Calender_FailConsistency_DF['Year'] = (Calender_FailConsistency_DF.loc[:,['Year']]).fillna(99999999)
        Calender_FailConsistency_DF['Year'] = (Calender_FailConsistency_DF.loc[:,['Year']]).replace([''], 99999999)
        Calender_FailConsistency_DF['Year'] = (Calender_FailConsistency_DF.loc[:,['Year']]).astype(int)

        Calender_FailConsistency_DF['Day'] = (Calender_FailConsistency_DF.loc[:,['Day']]).fillna(99999999)
        Calender_FailConsistency_DF['Day'] = (Calender_FailConsistency_DF.loc[:,['Day']]).replace([''], 99999999)
        Calender_FailConsistency_DF['Day'] = (Calender_FailConsistency_DF.loc[:,['Day']]).astype(int)

        Calender_FailConsistency_DF['Month'] = (Calender_FailConsistency_DF.loc[:,['Month']]).fillna(99999999)
        Calender_FailConsistency_DF['Month'] = (Calender_FailConsistency_DF.loc[:,['Month']]).replace([''], 99999999)
        Calender_FailConsistency_DF['Month'] = (Calender_FailConsistency_DF.loc[:,['Month']]).astype(int)

        Calender_FailConsistency_DF['HaulDay'] = (Calender_FailConsistency_DF.loc[:,['HaulDay']]).fillna(99999999)
        Calender_FailConsistency_DF['HaulDay'] = (Calender_FailConsistency_DF.loc[:,['HaulDay']]).replace([''], 99999999)
        Calender_FailConsistency_DF['HaulDay'] = (Calender_FailConsistency_DF.loc[:,['HaulDay']]).astype(int)

        Calender_FailConsistency_DF['HaulMonth'] = (Calender_FailConsistency_DF.loc[:,['HaulMonth']]).fillna(99999999)
        Calender_FailConsistency_DF['HaulMonth'] = (Calender_FailConsistency_DF.loc[:,['HaulMonth']]).replace([''], 99999999)
        Calender_FailConsistency_DF['HaulMonth'] = (Calender_FailConsistency_DF.loc[:,['HaulMonth']]).astype(int)

        Calender_FailConsistency_DF['StartTime'] = (Calender_FailConsistency_DF.loc[:,['StartTime']]).fillna(99999999)
        Calender_FailConsistency_DF['StartTime'] = (Calender_FailConsistency_DF.loc[:,['StartTime']]).replace([''], 99999999)
        Calender_FailConsistency_DF['StartTime'] = (Calender_FailConsistency_DF.loc[:,['StartTime']]).astype(int)

        Calender_FailConsistency_DF['Duration'] = (Calender_FailConsistency_DF.loc[:,['Duration']]).fillna(99999999.0)
        Calender_FailConsistency_DF['Duration'] = (Calender_FailConsistency_DF.loc[:,['Duration']]).replace([''], 99999999)
        Calender_FailConsistency_DF['Duration'] = (Calender_FailConsistency_DF.loc[:,['Duration']]).astype(float)

        Calender_FailConsistency_DF = Calender_FailConsistency_DF.replace([99999999, 99999999.0], '')
        Calender_FailConsistency_DF = Calender_FailConsistency_DF.reset_index(drop=True)
        Calender_FailConsistency_DF = pd.DataFrame(Calender_FailConsistency_DF)
        Calender_FailConsistency_DF.sort_values(by=['ASOCCode','DeploymentNumber','SetNumber',
                                                      'RecordType'], inplace=True)
        Calender_FailConsistency_DF  = Calender_FailConsistency_DF.reset_index(drop=True)
        Calender_FailConsistency_DF  = pd.DataFrame(Calender_FailConsistency_DF)      
        
        if len(Calender_FailConsistency_DF) >0:
            tree1.delete(*tree1.get_children())
            entry_ViewVarResults.current(0)
            txtDisplayMessageSystem.delete(0,END)
            txtDisplayMessageSystem.insert(0, 'Viewing QC Failed Excel File In Seperate Window')
            ExcelViewEditBackend_RecType_1_2(Calender_FailConsistency_DF)
    ...
    def Export_FailedConsistencyVariablesCSV():
        ...
        Calender_FailConsistency_DF = Calender_FailConsistency_DF.reset_index(drop=True)
        Complete_df = pd.DataFrame(Calender_FailConsistency_DF)
    ...
    def RefFailedToSetcatchDB():
        ...
        Calender_FailConsistency_DF = Calender_FailConsistency_DF.loc[:,
                        ['DataBase_ID','RecordIdentifier', 
                         'DeploymentUID']]
        Calender_FailConsistency_DF = Calender_FailConsistency_DF.reset_index(drop=True)
        Calender_FailConsistency_DF = pd.DataFrame(Calender_FailConsistency_DF)
        Merge_WithSetCatchDB=  GetSetCatchDB.merge(
                        Calender_FailConsistency_DF, 
        ...
        Merge_WithSetCatchDB[
            ['DataBase_ID','RecordIdentifier', 
             ...
             'Day', 'Month', 'HaulDay', 'HaulMonth']]= Merge_WithSetCatchDB[
            ['DataBase_ID','RecordIdentifier', 
             'ASOCCode','DeploymentNumber','SetNumber',
             'RecordType', 'Year','Country',
             'Day', 'Month', 'HaulDay', 'HaulMonth']].astype(int)
        ...
        Ref_FailedQC_InSetcatchDB.sort_values(by=['ASOCCode', 'DeploymentNumber',
    ...
    def SetcatchDB_VariableList():
        GetSetCatchDB_VariableList = ['Day', 'Month', 'HaulDay', 'HaulMonth']
    ...
    def Lookup_User_DB_Variable_UpdateDB(get_Updated_Variable, UpdateRecordList, 
        ...
        ListVariableSelect = ['Select Calender (Y-M-D/Y-HM-HD) Consistency View Type', 
                         'View Calender Consistency Fail With RecType-1,2',
                         'View Calender Consistency Fail With RecType-1 Only']
        conn_DB_Set_Catch_Analysis= sqlite3.connect(DB_Set_Catch_Analysis)
        cur_DB_Set_Catch_Analysis=conn_DB_Set_Catch_Analysis.cursor()
        ...
        if (get_entry_ViewVarResults==ListVariableSelect[1]):
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[0]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET Day = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET Day = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET QC_Message = ? \
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[1]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET Month = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET Month = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET QC_Message = ? \
                        WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                        SetNumber =? AND Year =? ",  
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[2]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET HaulDay = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET HaulDay = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET QC_Message = ? \
                        WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                        SetNumber =? AND Year =? ",  
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[3]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET HaulMonth = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET HaulMonth = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET QC_Message = ? \
                        WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                        SetNumber =? AND Year =? ",  
        ...
        if (get_entry_ViewVarResults==ListVariableSelect[2]):
            ## Updaing SetCatch DB
            if get_Updated_Variable == GetSetCatchDB_VariableList[0]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET Day = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET Day = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET QC_Message = ? \
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[1]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET Month = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET Month = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET QC_Message = ? \
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[2]:
                print(UpdateRecordList1)
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET HaulDay = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET HaulDay = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET QC_Message = ? \
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[3]:
                    cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET HaulMonth = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                    cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET HaulMonth = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                    cur_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET QC_Message = ? \
    ...
    def QCOnVariableType(get_Updated_Variable, get_UpdateValue_UpdatedVariable):
            ...
            Var_Class_IntA18=['Day', 'Month']
            Var_Class_IntA2 =['HaulDay', 'HaulMonth']
            ...
            if get_Updated_Variable in Var_Class_IntA2:
                if(len(get_UpdateValue_UpdatedVariable)!=0):
                    try:
                        get_UpdateValue_UpdatedVariable = int(get_UpdateValue_UpdatedVariable)
                        return get_UpdateValue_UpdatedVariable
                    except:
                        messagebox.showerror('Update Variable Datatype Error Message', "Updated Value Must Be Int Value")
                        return ReturnFail
                else:
    ...
    def callbackFuncSelectView(event):
        ...
        if(SelVariableView ==ListVariableListA[1]):
            tree1.delete(*tree1.get_children())
            txtDisplayMessageSystem.delete(0,END)
            txtDisplayMessageSystem.insert(1, 'Press Selected Results Button To View Calender (Y-M-D/Y-HM-HD) Fail In QC - RecType-1/2')
            
        if(SelVariableView ==ListVariableListA[2]):
            tree1.delete(*tree1.get_children())
            txtDisplayMessageSystem.delete(0,END)
            txtDisplayMessageSystem.insert(1, 'Press Selected Results Button To View Calender (Y-M-D/Y-HM-HD) Fail In QC - RecType-1 only')
    ...
    def GenSummaryQC():
        ...
        if len(Calender_FailConsistency_DF) >0:
            Summaryframetree.delete(*Summaryframetree.get_children())
            Calender_FailConsistency_DF = Calender_FailConsistency_DF.reset_index(drop=True)
            Summary_MG_FailConsistency_DF = Calender_FailConsistency_DF.groupby(
                ['ASOCCode', 'ObserverNumber','Year','DeploymentNumber'],  
    ...
    def InventoryRec3(event):
        ...
        Calender_FailConsistency_DF = pd.read_sql_query("SELECT * FROM SetCatch_DM_HDHM_FailConsistency_DF;", conn)
        Calender_FailConsistency_DF = pd.DataFrame(Calender_FailConsistency_DF)
        Calender_FailConsistency_DF = Calender_FailConsistency_DF[
                        (Calender_FailConsistency_DF.RecordType) == 1]
        Calender_FailConsistency_DF = Calender_FailConsistency_DF.reset_index(drop=True)
        Calender_FailConsistency_DF = pd.DataFrame(Calender_FailConsistency_DF)
        cursor.close()
        conn.close()
        ...
        if len(nm) ==1:
            ...
            checkinttype = isinstance(SelvariableIdentifier,int)
            if checkinttype == True:  
                ...
                if (int(NumberEntriesInSet) > 0):
                    ...
                    entry_SelQCVariable.delete(0,END)
                    Calender_FailConsistency_DF = Calender_FailConsistency_DF[
                        (Calender_FailConsistency_DF.DeploymentNumber) == SelvariableIdentifier]
                    Calender_FailConsistency_DF  = Calender_FailConsistency_DF.reset_index(drop=True)
                    Calender_FailConsistency_DF  = pd.DataFrame(Calender_FailConsistency_DF)
                    
                    if (len(Calender_FailConsistency_DF) < int(NumberEntriesInSet)) & \
                       (len(Calender_FailConsistency_DF) !=0):
                        tkinter.messagebox.showinfo("Null RecordType -1 Case","Atleast One Or Multi Null RecordType -1 Case In The Set")
                    
                    if (len(Calender_FailConsistency_DF) >0):
                        countIndex1 = 0
                        for each_rec in range(len(Calender_FailConsistency_DF)):
                            if countIndex1 % 2 == 0:
                                tree1.insert("", tk.END, values=list(Calender_FailConsistency_DF.loc[each_rec]), tags =("even",))
                            else:
                                tree1.insert("", tk.END, values=list(Calender_FailConsistency_DF.loc[each_rec]), tags =("odd",))
                            countIndex1 = countIndex1+1
                        tree1.tag_configure("even",foreground="black", background="lightgreen")
                        tree1.tag_configure("odd",foreground="black", background="ghost white")
                        entry_SelQCVariable.insert(tk.END,"C. QC View")

                        Cal_QCViewSelectDF= (Calender_FailConsistency_DF.loc[:,
                            ['DeploymentUID','RecordType','Year',
                            'Day','Month','HaulDay','HaulMonth']])
                        Cal_QCViewSelectDF  = Cal_QCViewSelectDF.reset_index(drop=True)
                        Cal_QCViewSelectDF  = pd.DataFrame(Cal_QCViewSelectDF)
                        countIndex1 = 0
                        for each_rec in range(len(Cal_QCViewSelectDF)):
                            if countIndex1 % 2 == 0:
                                SelResultOverviewtree.insert("", tk.END, values=list(Cal_QCViewSelectDF.loc[each_rec]), tags =("even",))
                            else:
                                SelResultOverviewtree.insert("", tk.END, values=list(Cal_QCViewSelectDF.loc[each_rec]), tags =("odd",))
                            countIndex1 = countIndex1+1
                        ...
                        tkinter.messagebox.showinfo("Null RecordType -1 Case","Null RecordType -1 Case In The Set")
                else:
                    txtDisplayMessageSystem.delete(0,END)
                ...
                if (get_DepNumforSearch) >= 0:
                    if (get_TabAQCview == ListFilterBy[0]):           
                        ...
                        rows= (rows.loc[:,
                            ...
                            'ASOCCode','ObserverNumber', 'DeploymentNumber', 
                            'SetNumber', 'RecordType', 'Year',
                            'Day', 'Month', 'HaulDay', 
                            'HaulMonth', 'StartTime', 'Duration']])
                        rows = rows[(rows.DeploymentNumber) == get_SearchSingleVariable_Value]
                        rows  = rows.reset_index(drop=True)
                        Calender_FailConsistency_DF  = pd.DataFrame(rows)
                        ExcelViewEditBackend_RecType_1_2(Calender_FailConsistency_DF)
                    ...
                    if (get_TabAQCview == ListFilterBy[1]):           
                        ...
                        rows  = rows.reset_index(drop=True)
                        Calender_FailConsistency_DF  = pd.DataFrame(rows)
                        ExcelViewEditBackend_RecType_1(Calender_FailConsistency_DF)
                conn_DB.commit()
                conn_DB.close()     

    def ExcelViewEditBackend_RecType_1_2(Calender_FailConsistency_DF):
        if len(Calender_FailConsistency_DF) >0:
            Calender_FailConsistency_DF.sort_values(by=['ASOCCode','DeploymentNumber','SetNumber',
                                                      'RecordType'], inplace=True)
            Calender_FailConsistency_DF  = Calender_FailConsistency_DF.reset_index(drop=True)
            ...
            frame = tk.Frame(windows)
            frame.pack(fill=BOTH, expand=1)
            pt = Table(frame, dataframe = Calender_FailConsistency_DF, showtoolbar=True, showstatusbar=True)
            pt.setRowColors(rows=range(1,len(Calender_FailConsistency_DF),2), clr='lightblue', cols='all')
            pt.cellbackgr = 'aliceblue'
            options = config.load_options()
            options = { 'align': 'center',
                        'cellbackgr': '#F4F4F3',
            ...
            def SubmitToUpdateDB():
                ...
                Complete_df= (Complete_df.loc[:,
                            ...
                            'Year','Day','Month', 
                            'HaulDay','HaulMonth',
                            'StartTime','Duration']]).replace(
                            ['', None, np.nan, 'None'], 99999999)
                Complete_df[['DataBase_ID','RecordIdentifier',
                            'ASOCCode','DeploymentNumber', 
                            'SetNumber', 'RecordType',
                            'Year','Day','Month', 
                            'HaulDay','HaulMonth', 'StartTime']] = Complete_df[
                            ['DataBase_ID','RecordIdentifier',
                            'ASOCCode','DeploymentNumber', 
                            'SetNumber', 'RecordType',
                            'Year','Day','Month', 
                            'HaulDay','HaulMonth', 'StartTime']].astype(int)
                Complete_df[['DeploymentUID','ObserverNumber']] = Complete_df[['DeploymentUID','ObserverNumber']].astype(str)
                Complete_df[['Duration']] = Complete_df[['Duration']].astype(float)
    ...
    def ExcelViewEditBackend_RecType_1(Calender_FailConsistency_DF):
        if len(Calender_FailConsistency_DF) >0:
            Calender_FailConsistency_DF.sort_values(by=['ASOCCode','DeploymentNumber','SetNumber',
                                                      'RecordType'], inplace=True)
            Calender_FailConsistency_DF  = Calender_FailConsistency_DF.reset_index(drop=True)
            ...
            def SubmitToUpdateDB():
                ...
                Complete_df[['DataBase_ID','RecordIdentifier',
                            ...
                            'Year','Day','Month', 
                            'HaulDay','HaulMonth', 'StartTime']].astype(int)
                Complete_df[['ObserverNumber', 'DeploymentUID']] = Complete_df[['ObserverNumber', 'DeploymentUID']].astype(str)
                Complete_df[['Duration']] = Complete_df[['Duration']].astype(float)
                Complete_df = Complete_df.replace([99999999, 99999999.0, '99999999'], '')
                if len(Complete_df) >0:
                    iSubmit = tkinter.messagebox.askyesno("Submit DFO-NL ASOP Set & Catch Edited Table",
            ...
            def BackendSubmitAndUpdateDB(Complete_df):
                ...
                cur_DB_SetCatch_Validation_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET \
                                        ASOCCode =?, ObserverNumber = ?,  DeploymentNumber =? , SetNumber =?,\
                                        Year =?, Day = ? , Month = ?, HaulDay = ?, HaulMonth = ?,\
        ...
        def UpdateQCFailDB():
            ...
            def GetQCFailDB():
                try:
                    conn = sqlite3.connect(DB_SetCatch_Validation_Consistency)
                    cursor = conn.cursor()
                    Complete_df = pd.read_sql_query("SELECT * FROM SetCatch_DM_HDHM_FailConsistency_DF ORDER BY `DataBase_ID` ASC ;", conn)
                    if len(Complete_df) >0:
                        SetCatchQCFailedDB_DF  = pd.DataFrame(Complete_df)
                        SetCatchQCFailedDB_DF = SetCatchQCFailedDB_DF.loc[:,['DataBase_ID','RecordIdentifier',
                                                                            'Year','ASOCCode','DeploymentNumber',
            ...
            cur_DB_Set_Catch_Consistency.executemany("UPDATE SetCatch_DM_HDHM_FailConsistency_DF SET DeploymentUID =?, Year = ?, \
                                                ASOCCode = ?, DeploymentNumber = ?, SetNumber = ?\
                                                WHERE DataBase_ID =? AND RecordIdentifier =?", 
                                                UpdateRecordList_SetCatchDB)
