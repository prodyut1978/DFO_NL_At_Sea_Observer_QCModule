...
def ViewConsis_MobileGear_ValResult():
    window = tk.Tk()
    window.title ("DFO-NL-ASOP Consistency Validator - ID-C-05-4")
    ...
    lbl_TopFrame = Label(TopFrame, font=('aerial', 10, 'bold'), bg= "cadet blue", text="A. QC Fail Table (Mobile Gear) :")
    lbl_TopFrame.grid(row =1, column = 0, padx=2, pady =1, ipady=1, sticky =W)

    ListVariableListA = ['Select MobileGear Consistency View Type', 
                         'View MobileGear Consistency Fail With RecType-1,2',
                         'View MobileGear Consistency Fail With RecType-1 Only']
    ...
    tree1 = ttk.Treeview(Tableframe, 
            column=("column1", "column2", "column3", 
                    ...
                    "column17"), height=22, show='headings')
    ...
    tree1.heading("#10", text="GearType", anchor=CENTER)
    tree1.heading("#11", text="Country", anchor=CENTER)
    tree1.heading("#12", text="Year", anchor=CENTER)
    tree1.heading("#13", text="Day", anchor=CENTER)
    tree1.heading("#14", text="Month", anchor=CENTER)
    tree1.heading("#15", text="StartTime", anchor=CENTER)
    tree1.heading("#16", text="Duration", anchor=CENTER)
    tree1.heading("#17", text="QC_Message", anchor=CENTER)
    ...
    tree1.column('#4', stretch=NO, minwidth=0, width=80, anchor = tk.CENTER)
    ...
    tree1.column('#12', stretch=NO, minwidth=0, width=60, anchor = tk.CENTER)            
    tree1.column('#13', stretch=NO, minwidth=0, width=60, anchor = tk.CENTER)
    tree1.column('#14', stretch=NO, minwidth=0, width=60, anchor = tk.CENTER)
    tree1.column('#15', stretch=NO, minwidth=0, width=80, anchor = tk.CENTER)            
    tree1.column('#16', stretch=NO, minwidth=0, width=80, anchor = tk.CENTER)
    tree1.column('#17', stretch=NO, minwidth=0, width=220, anchor = tk.CENTER)
    ...
    ListUpdateVar = ['Day', 'Month', 'StartTime', 'Duration']
...
## ######### Frame Generate QC Failed Summary ############
    ...
    lbl_SummaryQCframe = Label(SummaryQCframe, font=('aerial', 10, 'bold','underline'), 
                                bg= "cadet blue", text="B: QC Fail Summary (Mobile Gear)")
    ...
    Summaryframetree.heading("#2", text="Country")
    ...
    Summaryframetree.column('#2', stretch=NO, minwidth=0, width=72, anchor = tk.CENTER)
    Summaryframetree.column('#3', stretch=NO, minwidth=0, width=110, anchor = tk.CENTER)            
    ...
    QcValue = "Date-Time Stamp Of Current Set <= Date-Time Stamp Of Next Set Within Same Deployment"
    ...
    SelResultOverviewtree.heading("#6", text="StartTime", anchor=CENTER)
    SelResultOverviewtree.heading("#7", text="Duration", anchor=CENTER)
...
## ######All Defined Functions ####### 
                ...
                if filename.endswith('.csv'):
                    ...
                    if Return_Message == ReturnMatchedMessage:
                        ...
                        catdf.rename(columns=  {0:'DataBase_ID', 1: 'RecordIdentifier', 2: 'DeploymentUID',
                                                ...
                                                8:'RecordType', 9:'GearType', 10:'Country',
                                                11:'Year', 12:'Day', 13:'Month', 
                                                14:'StartTime', 15:'Duration'},inplace = True)
                    ...
                    if Len_CheckEmptyNessColumn==0:
                        ...
                        if Length_Raw_Imported_Df <250000:
                            ...
                            for row in df_rows:
                                ...
                                list_item_GearType = (rowValue[9])
                                list_item_Country = (rowValue[10])
                                list_item_Year = (rowValue[11])
                                list_item_Day = (rowValue[12])
                                list_item_Month = (rowValue[13])
                                list_item_StartTime = (rowValue[14])
                                list_item_Duration = (rowValue[15])
                                list_item_QCMsg = ''
                            ...
                            cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET ASOCCode = ?, \
                                                    ...
                                                    RecordType = ? , GearType = ? , Country = ?, Year = ?, Day = ?, Month =?,\
                                                    StartTime = ?, Duration =?\
                                                    WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                                                    UpdateRecordList_SetCatchDB)
                            
                            cur_DB_SetCatch_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF  SET ASOCCode = ?, \
                                                    ObserverNumber = ?, DeploymentNumber = ?, SetNumber = ?, VesselSideNumber =?,\
                                                    RecordType = ? , GearType = ? , Country = ?, Year = ?, Day = ?, Month =?,\
                                                    StartTime = ?, Duration =? , QC_Message =?\
                                                    WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
    ...
    def viewQCFailed(getVarnameToView):
        ...
        try:
            ...
            cur=con.cursor()
            cur.execute("SELECT * FROM SetCatch_MobileGear_FailConsistency_DF")
            rows=cur.fetchall()
            ...
            rows.rename(columns={0:'DataBase_ID', 1:'RecordIdentifier', 
                                 ...
                                 6:'SetNumber', 7:'VesselSideNumber', 
                                 8:'RecordType', 9:'GearType',
                                 10:'Country', 11:'Year', 
                                 12:'Day', 13:'Month',
                                 14:'StartTime', 15:'Duration', 
                                 16:'QC_Message'},inplace = True)
    ...
    def viewQCFailed_VariablesProfile():
        ...
        if len(rows) >0 :
            ...
            if getVarnameToView == ListVariableListA[0]:
                txtDisplayMessageSystem.delete(0,END)
                txtDisplayMessageSystem.insert(0, 'Select MG View Type From DropDown & Run View Selected Button')
            else:
                countIndex1 = 0
                ...
                txtDisplayMessageSystem.delete(0,END)
                entry_SelQCVariable.delete(0,END)
                txtDisplayMessageSystem.insert(0, 'Populated MobileGear Consistency Fail Entries')
        else:
            txtDisplayMessageSystem.delete(0,END)
    ...
    def QCFailedExcelViewAll():
        conn = sqlite3.connect(DB_SetCatch_Validation_Consistency)
        cursor = conn.cursor()
        MobileGear_FailConsistency_DF = pd.read_sql_query("SELECT * FROM SetCatch_MobileGear_FailConsistency_DF;", conn)
        MobileGear_FailConsistency_DF = pd.DataFrame(MobileGear_FailConsistency_DF)
        cursor.close()
        conn.close()

        MobileGear_FailConsistency_DF = pd.DataFrame(MobileGear_FailConsistency_DF)
        MobileGear_FailConsistency_DF['DataBase_ID'] = (MobileGear_FailConsistency_DF.loc[:,['DataBase_ID']]).astype(int)
        MobileGear_FailConsistency_DF['RecordIdentifier'] = (MobileGear_FailConsistency_DF.loc[:,['RecordIdentifier']]).astype(int)
        MobileGear_FailConsistency_DF['DeploymentUID'] = (MobileGear_FailConsistency_DF.loc[:,['DeploymentUID']]).astype(str)

        MobileGear_FailConsistency_DF['ASOCCode'] = (MobileGear_FailConsistency_DF.loc[:,['ASOCCode']]).fillna(99999999)
        MobileGear_FailConsistency_DF['ASOCCode'] = (MobileGear_FailConsistency_DF.loc[:,['ASOCCode']]).replace([''], 99999999)
        MobileGear_FailConsistency_DF['ASOCCode'] = (MobileGear_FailConsistency_DF.loc[:,['ASOCCode']]).astype(int)

        MobileGear_FailConsistency_DF['ObserverNumber'] = (MobileGear_FailConsistency_DF.loc[:,['ObserverNumber']]).astype(str)
        MobileGear_FailConsistency_DF['VesselSideNumber'] = (MobileGear_FailConsistency_DF.loc[:,['VesselSideNumber']]).astype(str)

        MobileGear_FailConsistency_DF['DeploymentNumber'] = (MobileGear_FailConsistency_DF.loc[:,['DeploymentNumber']]).fillna(99999999)
        MobileGear_FailConsistency_DF['DeploymentNumber'] = (MobileGear_FailConsistency_DF.loc[:,['DeploymentNumber']]).replace([''], 99999999)
        MobileGear_FailConsistency_DF['DeploymentNumber'] = (MobileGear_FailConsistency_DF.loc[:,['DeploymentNumber']]).astype(int)
        
        MobileGear_FailConsistency_DF['SetNumber'] = (MobileGear_FailConsistency_DF.loc[:,['SetNumber']]).fillna(99999999)
        MobileGear_FailConsistency_DF['SetNumber'] = (MobileGear_FailConsistency_DF.loc[:,['SetNumber']]).replace([''], 99999999)
        MobileGear_FailConsistency_DF['SetNumber'] = (MobileGear_FailConsistency_DF.loc[:,['SetNumber']]).astype(int)
        
        MobileGear_FailConsistency_DF['RecordType'] = (MobileGear_FailConsistency_DF.loc[:,['RecordType']]).fillna(99999999)
        MobileGear_FailConsistency_DF['RecordType'] = (MobileGear_FailConsistency_DF.loc[:,['RecordType']]).replace([''], 99999999)
        MobileGear_FailConsistency_DF['RecordType'] = (MobileGear_FailConsistency_DF.loc[:,['RecordType']]).astype(int)
        
        MobileGear_FailConsistency_DF['Year'] = (MobileGear_FailConsistency_DF.loc[:,['Year']]).fillna(99999999)
        MobileGear_FailConsistency_DF['Year'] = (MobileGear_FailConsistency_DF.loc[:,['Year']]).replace([''], 99999999)
        MobileGear_FailConsistency_DF['Year'] = (MobileGear_FailConsistency_DF.loc[:,['Year']]).astype(int)

        MobileGear_FailConsistency_DF['Day'] = (MobileGear_FailConsistency_DF.loc[:,['Day']]).fillna(99999999)
        MobileGear_FailConsistency_DF['Day'] = (MobileGear_FailConsistency_DF.loc[:,['Day']]).replace([''], 99999999)
        MobileGear_FailConsistency_DF['Day'] = (MobileGear_FailConsistency_DF.loc[:,['Day']]).astype(int)

        MobileGear_FailConsistency_DF['Month'] = (MobileGear_FailConsistency_DF.loc[:,['Month']]).fillna(99999999)
        MobileGear_FailConsistency_DF['Month'] = (MobileGear_FailConsistency_DF.loc[:,['Month']]).replace([''], 99999999)
        MobileGear_FailConsistency_DF['Month'] = (MobileGear_FailConsistency_DF.loc[:,['Month']]).astype(int)

        MobileGear_FailConsistency_DF['StartTime'] = (MobileGear_FailConsistency_DF.loc[:,['StartTime']]).fillna(99999999)
        MobileGear_FailConsistency_DF['StartTime'] = (MobileGear_FailConsistency_DF.loc[:,['StartTime']]).replace([''], 99999999)
        MobileGear_FailConsistency_DF['StartTime'] = (MobileGear_FailConsistency_DF.loc[:,['StartTime']]).astype(int)

        MobileGear_FailConsistency_DF['Duration'] = (MobileGear_FailConsistency_DF.loc[:,['Duration']]).fillna(99999999)
        MobileGear_FailConsistency_DF['Duration'] = (MobileGear_FailConsistency_DF.loc[:,['Duration']]).replace([''], 99999999)
        MobileGear_FailConsistency_DF['Duration'] = (MobileGear_FailConsistency_DF.loc[:,['Duration']]).astype(float)

        MobileGear_FailConsistency_DF = MobileGear_FailConsistency_DF.replace(99999999, '')
        MobileGear_FailConsistency_DF = MobileGear_FailConsistency_DF.reset_index(drop=True)
        MobileGear_FailConsistency_DF = pd.DataFrame(MobileGear_FailConsistency_DF)
        MobileGear_FailConsistency_DF.sort_values(by=['ASOCCode','DeploymentNumber','SetNumber',
                                                      'RecordType'], inplace=True)
        MobileGear_FailConsistency_DF  = MobileGear_FailConsistency_DF.reset_index(drop=True)
        MobileGear_FailConsistency_DF  = pd.DataFrame(MobileGear_FailConsistency_DF)      
        ...
        if len(MobileGear_FailConsistency_DF) >0:
            ...
            pt = Table(frame, dataframe = MobileGear_FailConsistency_DF, showtoolbar=True, showstatusbar=True)
            pt.setRowColors(rows=range(1,len(MobileGear_FailConsistency_DF),2), clr='lightblue', cols='all')
            ...
            def SubmitToUpdateDB():
                ...
                Complete_df= (Complete_df.loc[:,
                            ...
                            'ASOCCode','ObserverNumber', 'DeploymentNumber', 
                            'SetNumber', 'VesselSideNumber', 'RecordType', 'GearType', 'Country',
                            'Year','Day', 'Month', 'StartTime', 'Duration']]).replace(
                            ['', None, np.nan, 'None'], 99999999)
                Complete_df[['DataBase_ID','RecordIdentifier',
                            'ASOCCode','DeploymentNumber', 
                            'SetNumber', 'RecordType','GearType','Country',
                            'Year','Day','Month', 'StartTime']] = Complete_df[
                            ['DataBase_ID','RecordIdentifier',
                            'ASOCCode','DeploymentNumber', 
                            'SetNumber', 'RecordType','GearType','Country',
                            'Year','Day','Month', 'StartTime']].astype(int)
                ...
                def UpdateQCFailDB():
                    ...
                    def GetQCFailDB():
                        try:
                            conn = sqlite3.connect(DB_SetCatch_Validation_Consistency)
                            cursor = conn.cursor()
                            Complete_df = pd.read_sql_query("SELECT * FROM SetCatch_MobileGear_FailConsistency_DF ORDER BY `DataBase_ID` ASC ;", conn)
                            if len(Complete_df) >0:
                                SetCatchQCFailedDB_DF  = pd.DataFrame(Complete_df)
                                SetCatchQCFailedDB_DF = SetCatchQCFailedDB_DF.loc[:,['DataBase_ID','RecordIdentifier',
                                                                                    'Year','ASOCCode','DeploymentNumber',
                    ...
                    cur_DB_Set_Catch_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET DeploymentUID =?, Year = ?, \
                                                        ASOCCode = ?, DeploymentNumber = ?, SetNumber = ?\
                                                        WHERE DataBase_ID =? AND RecordIdentifier =?", 
                                                        UpdateRecordList_SetCatchDB)
    ...
    def Export_FailedConsistencyVariablesCSV():
        ...
        MobileGear_FailConsistency_DF['Country'] = (MobileGear_FailConsistency_DF.loc[:,['Country']]).fillna(99999999)
        MobileGear_FailConsistency_DF['Country'] = (MobileGear_FailConsistency_DF.loc[:,['Country']]).replace([''], 99999999)
        MobileGear_FailConsistency_DF['Country'] = (MobileGear_FailConsistency_DF.loc[:,['Country']]).astype(int)
        ...
        MobileGear_FailConsistency_DF = MobileGear_FailConsistency_DF.reset_index(drop=True)
        Complete_df = pd.DataFrame(MobileGear_FailConsistency_DF)
    ...
    def RefFailedToSetcatchDB():
        ...
        MobileGear_FailConsistency_DF = MobileGear_FailConsistency_DF.loc[:,
                        ['DataBase_ID','RecordIdentifier', 
                         'DeploymentUID']]
        MobileGear_FailConsistency_DF = MobileGear_FailConsistency_DF.reset_index(drop=True)
        MobileGear_FailConsistency_DF = pd.DataFrame(MobileGear_FailConsistency_DF)
        Merge_WithSetCatchDB=  GetSetCatchDB.merge(
                        MobileGear_FailConsistency_DF, 
        ...
        Merge_WithSetCatchDB[
            ['DataBase_ID','RecordIdentifier', 
             ...
             'Day', 'Month', 'StartTime']]= Merge_WithSetCatchDB[
            ['DataBase_ID','RecordIdentifier', 
             'ASOCCode','DeploymentNumber','SetNumber',
             'RecordType', 'Year','Country',
             'Day', 'Month', 'StartTime']].astype(int)
    ...
    def InventoryRec1(event):
        ...
        if len(curItems)==1:
            ...
            entry_SearchNeighbour.insert(tk.END,SelvariableUID)
            SelResultOverviewtree.delete(*SelResultOverviewtree.get_children())
    ...
    def SetcatchDB_VariableList():
        GetSetCatchDB_VariableList = ['Day', 'Month', 'StartTime', 'Duration']
    ...
    def Lookup_User_DB_Variable_UpdateDB(get_Updated_Variable, UpdateRecordList, 
        ...
        ListVariableSelect = ['Select MobileGear Consistency View Type', 
                         'View MobileGear Consistency Fail With RecType-1,2',
                         'View MobileGear Consistency Fail With RecType-1 Only']
        conn_DB_Set_Catch_Analysis= sqlite3.connect(DB_Set_Catch_Analysis)
        cur_DB_Set_Catch_Analysis=conn_DB_Set_Catch_Analysis.cursor()
        ...
        if (get_entry_ViewVarResults==ListVariableSelect[1]):
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[0]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET Day = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET Day = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET QC_Message = ? \
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[1]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET Month = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET Month = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET QC_Message = ? \
                        WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                        SetNumber =? AND Year =? ",  
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[2]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET StartTime = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET StartTime = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET QC_Message = ? \
                        WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                        SetNumber =? AND Year =? ",  
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[3]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET Duration = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET Duration = ? WHERE DataBase_ID =? AND RecordIdentifier =? AND DeploymentUID =?", 
                        UpdateRecordList)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET QC_Message = ? \
                        WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                        SetNumber =? AND Year =? ",  
        ...
        if (get_entry_ViewVarResults==ListVariableSelect[2]):
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[0]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET Day = ?\
                    ...
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET Day = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET QC_Message = ? \
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[1]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET Month = ?\
                    ...
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET Month = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET QC_Message = ? \
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[2]:
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET StartTime = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET StartTime = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET QC_Message = ? \
            ...
            if get_Updated_Variable == GetSetCatchDB_VariableList[3]:
                    cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET Duration = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                    cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET Duration = ?\
                    WHERE DeploymentUID =? AND ASOCCode =? AND DeploymentNumber =? AND \
                    SetNumber =? AND Year =? ", 
                    UpdateRecordList1)
                    cur_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET QC_Message = ? \
    ...
    def QCOnVariableType(get_Updated_Variable, get_UpdateValue_UpdatedVariable):
            ...
            Var_Class_IntA18=['Day', 'Month','StartTime']
    ...
    def callbackFuncSelectView(event):
        ...
        if(SelVariableView ==ListVariableListA[1]):
            tree1.delete(*tree1.get_children())
            txtDisplayMessageSystem.delete(0,END)
            txtDisplayMessageSystem.insert(1, 'Press Selected Results Button To View Mobile Gear (MG) Fail In QC - RecType-1/2')
            
        if(SelVariableView ==ListVariableListA[2]):
            tree1.delete(*tree1.get_children())
            txtDisplayMessageSystem.delete(0,END)
            txtDisplayMessageSystem.insert(1, 'Press Selected Results Button To View Mobile Gear (MG) Fail In QC - RecType-1 only')
    ...
    def GenSummaryQC():
        ...
        if len(MobileGear_FailConsistency_DF) >0:
            Summaryframetree.delete(*Summaryframetree.get_children())
            MobileGear_FailConsistency_DF = MobileGear_FailConsistency_DF.reset_index(drop=True)
            Summary_MG_FailConsistency_DF = MobileGear_FailConsistency_DF.groupby(['ASOCCode', 'Country','Year','DeploymentNumber'],  
    ...
    def InventoryRec3(event):
        ...
        MobileGear_FailConsistency_DF = pd.read_sql_query("SELECT * FROM SetCatch_MobileGear_FailConsistency_DF;", conn)
        MobileGear_FailConsistency_DF = pd.DataFrame(MobileGear_FailConsistency_DF)
        MobileGear_FailConsistency_DF = MobileGear_FailConsistency_DF[
                        (MobileGear_FailConsistency_DF.RecordType) == 1]
        MobileGear_FailConsistency_DF = MobileGear_FailConsistency_DF.reset_index(drop=True)
        MobileGear_FailConsistency_DF = pd.DataFrame(MobileGear_FailConsistency_DF)
        cursor.close()
        conn.close()
            ...
            if (checkinttype1 == True)& (checkinttype2 == True) &\
               (checkinttype3 == True)&(checkinttype4 == True):    
                ...
                if (int(NumberEntriesInSet) > 0):
                    ...
                    entry_SelQCVariable.delete(0,END)
                    MobileGear_FailConsistency_DF = MobileGear_FailConsistency_DF[
                        ((MobileGear_FailConsistency_DF.DeploymentNumber) == SelvariableIdentifier)&
                        ((MobileGear_FailConsistency_DF.ASOCCode) == SelvariableASOC)&
                        ((MobileGear_FailConsistency_DF.Country) == SelvariableCountry)&
                        ((MobileGear_FailConsistency_DF.Year) == SelvariableYr)
                        ]
                    MobileGear_FailConsistency_DF.sort_values(by=['DeploymentNumber'], inplace=True)
                    MobileGear_FailConsistency_DF  = MobileGear_FailConsistency_DF.reset_index(drop=True)
                    MobileGear_FailConsistency_DF  = pd.DataFrame(MobileGear_FailConsistency_DF)
                    
                    if (len(MobileGear_FailConsistency_DF) < int(NumberEntriesInSet)) & \
                       (len(MobileGear_FailConsistency_DF) !=0):
                        tkinter.messagebox.showinfo("Null RecordType -1 Case","Atleast One Or Multi Null RecordType -1 Case In The Set")
                    
                    if (len(MobileGear_FailConsistency_DF) >0):
                        countIndex1 = 0
                        for each_rec in range(len(MobileGear_FailConsistency_DF)):
                            if countIndex1 % 2 == 0:
                                tree1.insert("", tk.END, values=list(MobileGear_FailConsistency_DF.loc[each_rec]), tags =("even",))
                            else:
                                tree1.insert("", tk.END, values=list(MobileGear_FailConsistency_DF.loc[each_rec]), tags =("odd",))
                            countIndex1 = countIndex1+1
                        tree1.tag_configure("even",foreground="black", background="lightgreen")
                        tree1.tag_configure("odd",foreground="black", background="ghost white")
                        entry_SelQCVariable.insert(tk.END,"C. QC View")

                        MG_QCViewSelectDF= (MobileGear_FailConsistency_DF.loc[:,
                            ['DeploymentUID','RecordType','GearType',
                            'Day','Month','StartTime','Duration']])
                ...
                if (get_DepNumforSearch) >= 0:
                    if (get_TabAQCview == ListFilterBy[0]):           
                        ...
                        rows= (rows.loc[:,
                            ...
                            'ASOCCode','ObserverNumber', 'DeploymentNumber', 
                            'SetNumber', 'VesselSideNumber', 'RecordType', 'GearType', 'Country',
                            'Year','Day', 'Month', 'StartTime', 'Duration']])
                        rows = rows[(rows.DeploymentNumber) == get_SearchSingleVariable_Value]
                        rows  = rows.reset_index(drop=True)
                        MobileGear_FailConsistency_DF  = pd.DataFrame(rows)
                        ExcelViewEditBackend_RecType_1_2(MobileGear_FailConsistency_DF)
                    ...
                    if (get_TabAQCview == ListFilterBy[1]):           
                        ...
                        rows  = rows.reset_index(drop=True)
                        MobileGear_FailConsistency_DF  = pd.DataFrame(rows)
                        ExcelViewEditBackend_RecType_1(MobileGear_FailConsistency_DF)
                conn_DB.commit()
                conn_DB.close()     

    def ExcelViewEditBackend_RecType_1_2(MobileGear_FailConsistency_DF):
        if len(MobileGear_FailConsistency_DF) >0:
            MobileGear_FailConsistency_DF.sort_values(by=['ASOCCode','DeploymentNumber','SetNumber',
                                                    'RecordType'], inplace=True)
            MobileGear_FailConsistency_DF  = MobileGear_FailConsistency_DF.reset_index(drop=True)
    ...
    def ExcelViewEditBackend_RecType_1(MobileGear_FailConsistency_DF):
        if len(MobileGear_FailConsistency_DF) >0:
            MobileGear_FailConsistency_DF.sort_values(
                by=['ASOCCode','DeploymentNumber','SetNumber','RecordType'], inplace=True)
            MobileGear_FailConsistency_DF  = MobileGear_FailConsistency_DF.reset_index(drop=True)
            MobileGear_FailConsistency_DF  = pd.DataFrame(MobileGear_FailConsistency_DF)   
            txtDisplayMessageSystem.delete(0,END)
            txtDisplayMessageSystem.insert(0, 'Viewing Searched QC Failed Excel File In Seperate Window')
            ...
            def BackendSubmitAndUpdateDB(Complete_df):
                ...
                cur_DB_Set_Catch_Analysis.executemany("UPDATE DFO_NL_ASOP_Set_Catch_Analysis_IMPORT SET \
                                        ...
                                        VesselSideNumber = ?, GearType = ? ,Country =?,\
                                        Year =?, Day = ? , Month = ?, StartTime = ?, Duration = ?\
                                        WHERE DeploymentUID =?", 
                                        UpdateRecordList_SetCatchDB)
                
                cur_DB_SetCatch_Validation_Consistency.executemany("UPDATE SetCatch_MobileGear_FailConsistency_DF SET \
                                        ASOCCode =? , ObserverNumber = ?, DeploymentNumber =?, SetNumber =?, \
                                        VesselSideNumber = ?, GearType = ? , Country =?,\
                                        Year =?, Day = ? , Month = ?, StartTime = ?, Duration = ?, QC_Message =?\
                                        WHERE DeploymentUID = ?", 
                                        UpdateRecordList_QCfailDB)
    ...
    def SearchNeighborDepUID():
        ...
        try:
            get_Setnum_Split = int(get_Setnum_Split)
             ...
             messagebox.showerror('Search Variable Datatype Error Message', 
                                  "SetNumber Value Must Be Integer Value")
